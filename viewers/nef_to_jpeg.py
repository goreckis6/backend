#!/usr/bin/env python3
"""
NEF (Nikon RAW) to JPEG preview generator
Uses rawpy (LibRaw) to read NEF and outputs a JPEG preview, resized for web.
"""
...
def generate_preview(nef_file: str, output_file: str, max_dimension: int = 2048, quality: int = 85) -> bool:
    if not HAS_RAWPY or not HAS_PIL:
        print("ERROR: rawpy or Pillow not available")
        return False

    if not os.path.exists(nef_file):
        print(f"ERROR: input file not found: {nef_file}")
        return False
...
def main():
    parser = argparse.ArgumentParser(description='Generate JPEG preview from NEF (Nikon RAW)')
    parser.add_argument('nef_file', help='Path to input NEF file')
    ...
    ok = generate_preview(args.nef_file, args.output_file, args.max_dimension, args.quality)
