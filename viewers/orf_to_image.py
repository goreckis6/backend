#!/usr/bin/env python3
"""
ORF (Olympus RAW) to Image converter for web preview.
...
def extract_exif_metadata(orf_file):
    """Extract EXIF metadata from ORF file using exiftool."""
    metadata = {
        'dateTaken': 'N/A', 'dimensions': 'N/A', 'fileSize': 'N/A',
        'iso': 'N/A', 'camera': 'N/A', 'exposure': 'N/A'
    }
    try:
        file_size = os.path.getsize(orf_file)
        metadata['fileSize'] = f"{file_size / 1024 / 1024:.2f} MB"
        
        cmd = ['exiftool', '-json', '-DateTimeOriginal', '-ImageWidth', '-ImageHeight',
               '-ISO', '-Model', '-Make', '-ExposureTime', '-FNumber', '-FocalLength', orf_file]
...
def extract_embedded_jpeg(orf_file, output_file):
    """
    Fast preview using exiftool to extract embedded JPEG (no demosaic).
    
    Args:
        orf_file (str): Path to input ORF file
...
def convert_orf_with_rawpy(orf_file, output_file, max_dimension=2048):
    """
    ...
    """
    print(f"Processing ORF with rawpy (LibRaw)...")
    
    try:
        import rawpy
        
        # Open ORF file
        print("Opening ORF file with rawpy...")
        with rawpy.imread(orf_file) as raw:
            # Get RAW parameters
...
def convert_orf_to_image(orf_file, output_file, fast=True, max_dimension=2048):
    """
    Convert ORF to web-viewable image.
    ...
    Returns:
        bool: True if conversion successful, False otherwise
    """
    print(f"Starting ORF to Image conversion...")
    print(f"Input: {orf_file}")
    ...
    if not os.path.exists(orf_file):
        print(f"ERROR: Input ORF file not found: {orf_file}")
        return False
    
    file_size = os.path.getsize(orf_file)
    print(f"ORF file size: {file_size:,} bytes ({file_size / 1024 / 1024:.2f} MB)")
    
    # Try fast preview first if enabled
    if fast:
        if extract_embedded_jpeg(orf_file, output_file):
    ...
    return convert_orf_with_rawpy(orf_file, output_file, max_dimension)

def main():
    parser = argparse.ArgumentParser(description='Convert ORF (Olympus RAW) to web-viewable image')
    parser.add_argument('orf_file', help='Input ORF file path')
    ...
    metadata = extract_exif_metadata(args.orf_file)
