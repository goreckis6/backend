#!/usr/bin/env python3
"""
DNG (Adobe Digital Negative) to Image converter for web preview.
Uses rawpy (LibRaw), Pillow for rendering, and exiftool for fast embedded JPEG extraction.
Handles universal RAW format from any camera manufacturer.
"""
...
def extract_exif_metadata(dng_file):
    """Extract EXIF metadata from DNG file using exiftool."""
    metadata = {
    ...
    try:
        ...
        cmd = ['exiftool', '-json', '-DateTimeOriginal', '-ImageWidth', '-ImageHeight',
               '-ISO', '-Model', '-Make', '-ExposureTime', '-FNumber', '-FocalLength', dng_file]
...
def extract_embedded_jpeg(dng_file, output_file):
    """
...
def convert_dng_with_rawpy(dng_file, output_file, max_dimension=2048):
    """
    Full RAW processing with rawpy (LibRaw) and Pillow.
    Handles universal DNG format from any camera.
    ...
    """
    print(f"Processing DNG with rawpy (LibRaw)...")
    ...
    try:
        ...
        with rawpy.imread(dng_file) as raw:
            ...
            print("Processing universal RAW data (demosaic, white balance, color correction)...")
            rgb = raw.postprocess(
            ...
            if os.path.exists(output_file):
                file_size = os.path.getsize(output_file)
                print(f"Universal RAW processed image created: {file_size} bytes")
                return True
            else:
...
def convert_dng_to_image(dng_file, output_file, fast=True, max_dimension=2048):
    """
    Convert DNG to web-viewable image.
    ...
    Returns:
        bool: True if conversion successful, False otherwise
    """
    print(f"Starting DNG to Image conversion...")
    ...
    if not os.path.exists(dng_file):
        print(f"ERROR: Input DNG file not found: {dng_file}")
        return False
    
    file_size = os.path.getsize(dng_file)
    print(f"DNG file size: {file_size:,} bytes ({file_size / 1024 / 1024:.2f} MB)")
    
    # Try fast preview first if enabled
    if fast:
        if extract_embedded_jpeg(dng_file, output_file):
    ...
    return convert_dng_with_rawpy(dng_file, output_file, max_dimension)

def main():
    parser = argparse.ArgumentParser(description='Convert DNG (Adobe Digital Negative) to web-viewable image')
    ...
    metadata = extract_exif_metadata(args.dng_file)
