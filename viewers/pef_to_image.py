#!/usr/bin/env python3
"""
PEF (Pentax Electronic File) to Image converter for web preview.
...
def extract_exif_metadata(pef_file):
    """Extract EXIF metadata from PEF file using exiftool."""
    metadata = {
        'dateTaken': 'N/A', 'dimensions': 'N/A', 'fileSize': 'N/A',
        'iso': 'N/A', 'camera': 'N/A', 'exposure': 'N/A'
    }
    try:
        file_size = os.path.getsize(pef_file)
        metadata['fileSize'] = f"{file_size / 1024 / 1024:.2f} MB"
        
        cmd = ['exiftool', '-json', '-DateTimeOriginal', '-ImageWidth', '-ImageHeight',
               '-ISO', '-Model', '-Make', '-ExposureTime', '-FNumber', '-FocalLength', pef_file]
...
def extract_embedded_jpeg(pef_file, output_file):
    """
    Fast preview using exiftool to extract embedded JPEG (no demosaic).
    
    Args:
        pef_file (str): Path to input PEF file
...
def convert_pef_with_rawpy(pef_file, output_file, max_dimension=2048):
    """
    ...
    """
    print(f"Processing PEF with rawpy (LibRaw)...")
    
    try:
        import rawpy
        
        # Open PEF file
        print("Opening PEF file with rawpy...")
        with rawpy.imread(pef_file) as raw:
            # Get RAW parameters
...
def convert_pef_to_image(pef_file, output_file, fast=True, max_dimension=2048):
    """
    Convert PEF to web-viewable image.
    ...
    Returns:
        bool: True if conversion successful, False otherwise
    """
    print(f"Starting PEF to Image conversion...")
    print(f"Input: {pef_file}")
    print(f"Output: {output_file}")
    print(f"Fast mode: {fast}")
    print(f"Max dimension: {max_dimension}")
    
    # Check if input file exists
    if not os.path.exists(pef_file):
        print(f"ERROR: Input PEF file not found: {pef_file}")
        return False
    
    file_size = os.path.getsize(pef_file)
    print(f"PEF file size: {file_size:,} bytes ({file_size / 1024 / 1024:.2f} MB)")
    
    # Try fast preview first if enabled
    if fast:
        if extract_embedded_jpeg(pef_file, output_file):
    ...
    return convert_pef_with_rawpy(pef_file, output_file, max_dimension)

def main():
    parser = argparse.ArgumentParser(description='Convert PEF (Pentax Electronic File) to web-viewable image')
    parser.add_argument('pef_file', help='Input PEF file path')
    ...
    metadata = extract_exif_metadata(args.pef_file)
