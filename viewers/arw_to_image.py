#!/usr/bin/env python3
"""
ARW (Sony RAW) to Image converter for web preview.
...
def extract_exif_metadata(arw_file):
    """Extract EXIF metadata from ARW file using exiftool."""
    metadata = {
        'dateTaken': 'N/A', 'dimensions': 'N/A', 'fileSize': 'N/A',
        'iso': 'N/A', 'camera': 'N/A', 'exposure': 'N/A'
    }
    try:
        file_size = os.path.getsize(arw_file)
        metadata['fileSize'] = f"{file_size / 1024 / 1024:.2f} MB"
        
        cmd = ['exiftool', '-json', '-DateTimeOriginal', '-ImageWidth', '-ImageHeight',
               '-ISO', '-Model', '-Make', '-ExposureTime', '-FNumber', '-FocalLength', arw_file]
...
def extract_embedded_jpeg(arw_file, output_file):
    """
    Fast preview using exiftool to extract embedded JPEG (no demosaic).
    
    Args:
        arw_file (str): Path to input ARW file
...
def convert_arw_with_rawpy(arw_file, output_file, max_dimension=2048):
    """
    ...
    """
    print(f"Processing ARW with rawpy (LibRaw)...")
    
    try:
        import rawpy
        
        # Open ARW file
        print("Opening ARW file with rawpy...")
...
def convert_arw_to_image(arw_file, output_file, fast=True, max_dimension=2048):
    """
    Convert ARW to web-viewable image.
    ...
    Returns:
        bool: True if conversion successful, False otherwise
    """
    print(f"Starting ARW to Image conversion...")
    print(f"Input: {arw_file}")
    ...
    if not os.path.exists(arw_file):
        print(f"ERROR: Input ARW file not found: {arw_file}")
        return False
    
    file_size = os.path.getsize(arw_file)
    print(f"ARW file size: {file_size:,} bytes ({file_size / 1024 / 1024:.2f} MB)")
    
    # Try fast preview first if enabled
    if fast:
        if extract_embedded_jpeg(arw_file, output_file):
    ...
    return convert_arw_with_rawpy(arw_file, output_file, max_dimension)

def main():
    parser = argparse.ArgumentParser(description='Convert ARW (Sony RAW) to web-viewable image')
    parser.add_argument('arw_file', help='Input ARW file path')
    ...
    metadata = extract_exif_metadata(args.arw_file)
