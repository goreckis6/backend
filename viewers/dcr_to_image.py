#!/usr/bin/env python3
"""
DCR (Kodak RAW) to Image converter for web preview.
...
def extract_embedded_jpeg(dcr_file, output_file):
    ...
    Args:
        dcr_file (str): Path to input DCR file
...
def convert_dcr_with_rawpy(dcr_file, output_file, max_dimension=2048):
    """
    ...
    """
    print(f"Processing DCR with rawpy (LibRaw)...")
    
    try:
        import rawpy
        
        # Open DCR file
        print("Opening DCR file with rawpy...")
        with rawpy.imread(dcr_file) as raw:
            # Get RAW parameters
...
def convert_dcr_to_image(dcr_file, output_file, fast=True, max_dimension=2048):
    """
    Convert DCR to web-viewable image.
    ...
    Returns:
        bool: True if conversion successful, False otherwise
    """
    print(f"Starting DCR to Image conversion...")
    print(f"Input: {dcr_file}")
    ...
    if not os.path.exists(dcr_file):
        print(f"ERROR: Input DCR file not found: {dcr_file}")
        return False
    
    file_size = os.path.getsize(dcr_file)
    print(f"DCR file size: {file_size:,} bytes ({file_size / 1024 / 1024:.2f} MB)")
    
    # Try fast preview first if enabled
    if fast:
        if extract_embedded_jpeg(dcr_file, output_file):
    ...
    return convert_dcr_with_rawpy(dcr_file, output_file, max_dimension)
