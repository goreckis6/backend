#!/usr/bin/env python3
"""
STI (StarOffice Presentation Template) to HTML converter for web preview.
Uses LibreOffice for conversion. Handles legacy StarOffice/StarImpress template format.
"""
...
def convert_sti_to_html_libreoffice(sti_file, html_file):
    """
    Convert STI to HTML using LibreOffice.
    
    Args:
        sti_file (str): Path to input STI file
        html_file (str): Path to output HTML file
    
    Returns:
        bool: True if conversion successful, False otherwise
    """
    print(f"Attempting STI to HTML conversion with LibreOffice...")
    ...
    try:
        ...
        base_name = os.path.splitext(os.path.basename(sti_file))[0]
...
def main():
    parser = argparse.ArgumentParser(description='Convert STI to HTML for web preview')
    parser.add_argument('sti_file', help='Input STI file path')
    parser.add_argument('html_file', help='Output HTML file path')
    
    args = parser.parse_args()
    
    print("=== STI to HTML Converter ===")
    print(f"Python version: {sys.version}")
    print(f"Working directory: {os.getcwd()}")
    print(f"Arguments: {vars(args)}")
    
    # Check if input file exists
    if not os.path.exists(args.sti_file):
        print(f"ERROR: Input STI file not found: {args.sti_file}")
        sys.exit(1)
    
    print("Detected format: STI (StarOffice Presentation Template - Legacy)")
    ...
    success = convert_sti_to_html_libreoffice(args.sti_file, args.html_file)
