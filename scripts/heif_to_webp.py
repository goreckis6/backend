#!/usr/bin/env python3
"""
HEIF/HEIC to WebP Converter
Reads HEIF/HEIC via pillow-heif and saves WebP using Pillow.
...
def convert_heif_to_webp(heif_file: str, output_file: str, quality: int = 90, max_dimension: int = 4096) -> bool:
    print("Starting HEIF/HEIC to WebP conversion")
    ...
    try:
        ...
        # WebP supports transparency - preserve RGBA mode if present
        ...
        img.save(output_file, format='WebP', quality=max(1, min(100, int(quality))), method=6)
        ...
        print(f"ERROR: Failed to convert HEIF/HEIC to WebP: {e}")
        traceback.print_exc()
        return False


def main():
    parser = argparse.ArgumentParser(description='Convert HEIF/HEIC image to WebP')
    ...
    print("=== HEIF/HEIC to WebP Converter ===")
    print(f"Python: {sys.version}")
    print(f"Args: {vars(args)}")

    ok = convert_heif_to_webp(args.heif_file, args.output_file, args.quality, args.max_dimension)
