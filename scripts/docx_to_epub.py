...
Converts Microsoft Word DOCX files to EPUB format
Uses python-docx to extract content and ebooklib to create EPUB
"""
...
def convert_docx_to_epub(docx_file, output_file, include_images=True, preserve_formatting=True, generate_toc=True):
    """
    ...
    """
    print(f"Starting DOCX to EPUB conversion...")
    ...
    try:
        ...
        # Get title from filename if not found
        ...
        images_dir = os.path.join(os.path.dirname(output_file), f"images_{os.path.basename(output_file)}")
...
<body>
    ...
    {chapter_html}
        ...
        print(f"ERROR: Failed to convert DOCX to EPUB: {e}")
        traceback.print_exc()
        return False


def main():
    parser = argparse.ArgumentParser(description='Convert DOCX file to EPUB format')
    ...
    print("=== DOCX to EPUB Converter ===")
    print(f"Python version: {sys.version}")
    print(f"Working directory: {os.getcwd()}")
    print(f"Arguments: {vars(args)}")
    
    success = convert_docx_to_epub(
        args.docx_file, 
        args.output_file,
