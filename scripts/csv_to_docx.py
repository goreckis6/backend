...
def csv_to_docx(csv_file_path, docx_file_path, title="CSV Data Document", author="Generated by Morphy", include_date=True):
    ...
    if not df.empty:
        ...
        for i, col_name in enumerate(df.columns):
            ...
            # Set header cell background color (dark blue)
            hdr_cells[i]._tc.get_or_add_tcPr().set_tcBorders(
                top=qn('w:nil'), bottom=qn('w:nil'), start=qn('w:nil'), end=qn('w:nil'),
                left=qn('w:nil'), right=qn('w:nil')
            )
            shading_elm = qn('w:shd')
            shading_attrs = {qn('w:val'): 'clear', qn('w:color'): 'auto', qn('w:fill'): '4472C4'} # Dark blue fill
            hdr_cells[i]._tc.get_or_add_tcPr().insert_after(
                qn('w:shd', shading_attrs)
            )
