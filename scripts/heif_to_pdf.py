#!/usr/bin/env python3
"""
HEIF/HEIC to PDF Converter
Reads HEIF/HEIC via pillow-heif and saves PDF using Pillow.
...
def convert_heif_to_pdf(heif_file: str, output_file: str, quality: int = 90, max_dimension: int = 4096) -> bool:
    print("Starting HEIF/HEIC to PDF conversion")
    ...
    try:
        ...
        # PDF requires RGB mode; flatten transparency on white background
        if img.mode in ("RGBA", "LA") or 'transparency' in img.info:
            print("Flattening transparency over white for PDF")
        ...
        # Save as PDF
        img.save(output_file, format='PDF', quality=max(1, min(100, int(quality))), optimize=True)

        if os.path.exists(output_file) and os.path.getsize(output_file) > 0:
            print("PDF created successfully")
            return True
        print("ERROR: PDF output not created")
        return False
    except Exception as e:
        print(f"ERROR: Failed to convert HEIF/HEIC to PDF: {e}")
        traceback.print_exc()
        return False


def main():
    parser = argparse.ArgumentParser(description='Convert HEIF/HEIC image to PDF')
    ...
    parser.add_argument('--quality', type=int, default=90, help='PDF quality (1-100)')
    parser.add_argument('--max-dimension', type=int, default=4096, help='Max width or height for downscaling')
    args = parser.parse_args()

    print("=== HEIF/HEIC to PDF Converter ===")
    print(f"Python: {sys.version}")
    print(f"Args: {vars(args)}")

    ok = convert_heif_to_pdf(args.heif_file, args.output_file, args.quality, args.max_dimension)
