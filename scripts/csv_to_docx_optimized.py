...
def process_chunk_multiprocess(chunk_df, chunk_info):
    ...
    Returns processed data ready for DOCX insertion.
    """
    try:
        print(f"Process {os.getpid()} processing chunk {chunk_info}: {len(chunk_df)} rows")
        
        # Convert chunk to list of lists for DOCX table
        chunk_rows = []
...
def create_docx_from_csv_optimized(csv_file, output_file, title="CSV Data", author="Unknown", chunk_size=500, use_multiprocessing=True):
    """
    Optimized CSV to DOCX conversion with performance improvements and multiprocessing.
