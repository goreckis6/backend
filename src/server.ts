...
app.post('/api/preview/x3f', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'morphy-x3f-preview-'));
    const inputPath = path.join(tmpDir, file.originalname);
    ...
    const styledHtml = `
      ...
      <body>
        ...
        <script>
          ...
          // Hide metadata elements (Rows, Columns, File Size, Status, Type, Keys, Sheet labels)
          function hideMetadata() {
            const contentIds = ['json-content', 'ods-content', 'xlsx-content', 'csv-content', 'xml-content', 'js-content', 'html-content'];
            let content = null;
            ...
            // Remove elements containing metadata labels - be more aggressive
            const allElements = Array.from(content.querySelectorAll('*'));
            
            allElements.forEach(el => {
              const text = (el.textContent || '').trim();
              
              // Pattern matching for metadata sections
              const metadataPatterns = [
                /^Sheet\d+$/i,
                /^Rows\s*:?\s*\d+[,.]?\d*$/i,
                /^Columns\s*:?\s*\d+$/i,
                /^File Size\s*:?\s*[\d.,]+\s*(KB|MB|GB|Bytes?)$/i,
                /^Status\s*:?\s*Valid\s*[✓✓]$/i,
                /^Type\s*:?\s*(Object|Array|String|Number)$/i,
                /^Keys\s*:?\s*\d+$/i,
                /^Lines\s*:?\s*\d+$/i,
                /^Functions\s*:?\s*\d+$/i,
                /^Elements\s*:?\s*\d+$/i,
                /^DOCTYPE\s*:?\s*(Yes|No)\s*[✓✓]?$/i,
                /Rows[\s:]*\d+.*Columns[\s:]*\d+/i,
                /File Size[\s:]*[\d.,]+\s*(KB|MB|GB)/i,
                /Status[\s:]*Valid/i,
                /Lines[\s:]*\d+/i,
                /Functions[\s:]*\d+/i,
                /Elements[\s:]*\d+/i,
                /Top Tags.*html.*head.*meta/i
              ];
              
              // Check if this element matches metadata patterns
              const isMetadata = metadataPatterns.some(pattern => pattern.test(text)) ||
                (text.length < 100 && (
                  text.includes('Rows') && text.includes(':') ||
                  text.includes('Columns') && text.includes(':') ||
                  text.includes('File Size') ||
                  text.includes('Status') && text.includes('Valid') ||
                  text.includes('Type') && (text.includes('Object') || text.includes('Array')) ||
                  text.includes('Keys') && text.includes(':') ||
                  text.includes('Lines') && text.includes(':') ||
                  text.includes('Functions') && text.includes(':') ||
                  text.includes('Elements') && text.includes(':') ||
                  text.includes('DOCTYPE') && (text.includes('Yes') || text.includes('No')) ||
                  text.includes('Top Tags') ||
                  /^Sheet\d+$/.test(text)
                ));
              
              if (isMetadata) {
                // Check if parent also looks like metadata container
                const parent = el.parentElement;
                if (parent && parent !== content) {
                  const parentText = (parent.textContent || '').trim();
                  if (parentText.length < 200 && (
                    parentText.includes('Rows') || 
                    parentText.includes('Columns') || 
                    parentText.includes('File Size') ||
                    parentText.includes('Status') ||
                    parentText.includes('Type') ||
                    parentText.includes('Keys') ||
                    parentText.includes('Lines') ||
                    parentText.includes('Functions') ||
                    parentText.includes('Elements') ||
                    parentText.includes('DOCTYPE') ||
                    parentText.includes('Top Tags') ||
                    /Sheet\d+/.test(parentText)
                  )) {
            ...
            // Remove empty or very thin divs (gray frame lines)
            const allDivs = Array.from(content.querySelectorAll('div'));
            allDivs.forEach(div => {
              ...
              // Remove thin horizontal lines (1-3px height)
              if (height > 0 && height <= 3 && text.length < 10) {
                div.style.display = 'none';
                return;
              }
              
              // Remove gray frame backgrounds that don't contain substantial content
              if (bgColor && (
                bgColor.includes('rgb(248') || 
                bgColor.includes('rgb(249') ||
                bgColor.includes('rgb(250') ||
                bgColor.includes('rgb(251') ||
                bgColor.includes('gray') ||
                bgColor.includes('lightgray')
              )) {
                const hasSubstantialContent = div.querySelector('table, pre, code, ol, ul') || 
                                             (text.length > 100 && !text.match(/^(Rows|Columns|File Size|Status|Type|Keys|Lines|Functions|Elements|DOCTYPE|Top Tags|Sheet\d+|JavaScript Code|HTML File)/i));
                if (!hasSubstantialContent && text.length < 50) {
                  div.style.display = 'none';
                }
              }
            });
            ...
            const emptyContainers = Array.from(content.querySelectorAll('div'));
            emptyContainers.forEach(div => {
              if (div.style.display === 'none') return;
              const text = (div.textContent || '').trim();
              const children = div.children;
              
              // Remove divs that only contain whitespace or metadata labels
              if (text.length === 0 || (text.length < 20 && /^(Sheet\d+|Rows|Columns|File Size|Status|Type|Keys|Lines|Functions|Elements|DOCTYPE|Top Tags|JavaScript Code|HTML File)/i.test(text))) {
                if (children.length === 0 || Array.from(children).every(c => c.style.display === 'none' || !c.textContent.trim())) {
                  div.style.display = 'none';
                }
              }
            });
          ...
          // Run after content loads (multiple attempts to catch dynamically loaded content)
          setTimeout(hideMetadata, 100);
          setTimeout(hideMetadata, 500);
          setTimeout(hideMetadata, 1000);
        </script>
      </body>
      </html>
    `;
...
app.post('/api/preview/python', uploadDocument.single('file'), async (req, res) => {
  ...
  try {
    ...
    // Remove metadata/info sections for JavaScript (File Size, Lines, Functions, etc.)
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?(?:File Size|Lines|Functions|Status|Type|Keys|Valid|Elements|DOCTYPE|Top Tags)[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?(?:JavaScript Code|HTML File)[\s\S]*?Preview[\s\S]*?<\/div>/gi, '');
    ...
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?Lines[\s\S]*?\d+[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?Functions[\s\S]*?\d+[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?Elements[\s\S]*?\d+[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?DOCTYPE[\s\S]*?(?:Yes|No)[\s\S]*?✓?[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?Top Tags[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/(?:\{[^}]*\}|\{[\s]*\})?[\s]*(?:JavaScript Code|HTML File)[\s\S]*?Preview[\s]*(?:🖨️\s*Print|Print)[\s]*(?:✖️\s*Close|Close)/gi, '');
    
    // Remove gray frames/borders - same as other viewers
...
app.post('/api/preview/json', uploadDocument.single('file'), async (req, res) => {
  ...
  try {
    ...
    // Remove metadata/info sections (Rows, Columns, File Size, Status, Type, Keys, etc.)
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?(?:Rows|Columns|File Size|Status|Type|Keys|Valid)[\s\S]*?(?:\d+|✓|✓|Object|Array)[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?Rows[\s\S]*?\d+[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?Columns[\s\S]*?\d+[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?File Size[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?Status[\s\S]*?Valid[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?Type[\s\S]*?(?:Object|Array|String|Number)[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?Keys[\s\S]*?\d+[\s\S]*?<\/div>/gi, '');
    
    // Remove table rows/cells with metadata
    htmlContent = htmlContent.replace(/<tr[^>]*>[\s\S]*?(?:Rows|Columns|File Size|Status|Type|Keys)[\s\S]*?<\/tr>/gi, '');
    htmlContent = htmlContent.replace(/<td[^>]*>[\s\S]*?(?:Rows|Columns|File Size|Status|Type|Keys)[\s\S]*?<\/td>/gi, '');
    htmlContent = htmlContent.replace(/<th[^>]*>[\s\S]*?(?:Rows|Columns|File Size|Status|Type|Keys)[\s\S]*?<\/th>/gi, '');
    
    // Remove info boxes/panels with stats
    htmlContent = htmlContent.replace(/<div[^>]*class[^>]*["'][^"']*(?:info|metadata|stats|statistics|panel|box)[^"']*["'][^>]*>[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*id[^>]*["'][^"']*(?:info|metadata|stats|statistics|panel|box)[^"']*["'][^>]*>[\s\S]*?<\/div>/gi, '');
    
    // Remove spans/p tags with metadata labels
    htmlContent = htmlContent.replace(/<span[^>]*>[\s\S]*?(?:Rows|Columns|File Size|Status|Type|Keys)[\s\S]*?<\/span>/gi, '');
    htmlContent = htmlContent.replace(/<p[^>]*>[\s\S]*?(?:Rows|Columns|File Size|Status|Type|Keys)[\s\S]*?<\/p>/gi, '');
    htmlContent = htmlContent.replace(/<label[^>]*>[\s\S]*?(?:Rows|Columns|File Size|Status|Type|Keys)[\s\S]*?<\/label>/gi, '');
    
    // Remove strong/bold tags with metadata
    htmlContent = htmlContent.replace(/<strong[^>]*>[\s\S]*?(?:Rows|Columns|File Size|Status|Type|Keys)[\s\S]*?<\/strong>/gi, '');
    htmlContent = htmlContent.replace(/<b[^>]*>[\s\S]*?(?:Rows|Columns|File Size|Status|Type|Keys)[\s\S]*?<\/b>/gi, '');
    
    // Remove any divs containing both label and value patterns (e.g., "Rows: 1,000")
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?(?:Rows|Columns)[\s\S]*?:[\s\S]*?\d+[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?(?:File Size|Status|Type|Keys)[\s\S]*?:[\s\S]*?<\/div>/gi, '');
    
    // Remove remaining metadata containers
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?[\d,]+[\s\S]*?(?:KB|MB|GB|Bytes)[\s\S]*?<\/div>/gi, '');
    
    // Remove Sheet labels and surrounding elements more aggressively
    htmlContent = htmlContent.replace(/<div[^>]*>[\s\S]*?Sheet\d+[\s\S]*?<\/div>/gi, '');
    htmlContent = htmlContent.replace(/<[^>]*>[\s\S]*?Sheet\d+[\s\S]*?<\/[^>]*>/gi, '');
      ...
      <head>
        ...
        <style>
          * {
          ...
          /* Hide metadata elements using CSS - target common patterns */
          .json-content div[style*="background"] div,
          .json-content div[style*="gray"],
          .json-content div[style*="#f"] {
      ...
      <head>
        ...
        <style>
          * {
          ...
          .ods-content, .xlsx-content, .csv-content, .xml-content {
            background: white;
            border: 1px solid #d0d7de;
            border-radius: 8px;
            padding: 20px;
...
app.post('/convert/heif-to-png/single', upload.single('file'), async (req, res) => {
  res.set({
    'Access-Control-Allow-Origin': '*',
    'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
    'Access-Control-Allow-Headers': 'Content-Type, Authorization, Accept'
        ...
        await new Promise<void>((resolve) => {
          python.on('close', async (code: number) => {
            try {
              if (code === 0 && await fs.access(outputPath).then(() => true).catch(() => false)) {
                const outputBuffer = await fs.readFile(outputPath);
                results.push({ originalName: file.originalname, outputFilename: path.basename(outputPath), size: outputBuffer.length, success: true, downloadPath: `data:image/png;base64,${outputBuffer.toString('base64')}` });
              } else {
                results.push({ originalName: file.originalname, outputFilename: '', size: 0, success: false, error: stderr || `Conversion failed with code ${code}` });
              }
            } finally { resolve(); }
