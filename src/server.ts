...
// Route: DNG to ICO (Single)
app.post('/convert/dng-to-ico/single', upload.single('file'), async (req, res) => {
  // Set CORS headers
  res.set({
    'Access-Control-Allow-Origin': '*',
    'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
  ...
  console.log('DNG->ICO single conversion request');
  ...
  // Handle timeout gracefully with CORS headers
  const timeoutHandler = () => {
    console.log('DNG to ICO: Request timeout - sending timeout response');
    res.set({
      'Access-Control-Allow-Origin': '*',
      'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
      'Access-Control-Allow-Headers': 'Content-Type, Authorization, Accept'
    ...
    if (!res.headersSent) {
      res.status(408).json({ 
        error: 'Conversion timeout', 
        message: 'DNG to ICO conversion is taking longer than expected. Please try with a smaller file or contact support.',
        timeout: true
      });
    }
  };
  ...
  const tmpDir = path.join(os.tmpdir(), `dng-ico-${Date.now()}`);
  ...
  try {
    ...
    const inputPath = path.join(tmpDir, file.originalname);
    const outputPath = path.join(tmpDir, file.originalname.replace(/\.dng$/i, '.ico'));

    await fs.writeFile(inputPath, file.buffer);

    // Get icon size from request body (use original size if not specified)
    const iconSize = req.body.iconSize;
    console.log('DNG to ICO: Icon size received:', iconSize, 'Type:', typeof iconSize);
    console.log('DNG to ICO: Icon size check (iconSize !== "default"):', iconSize !== 'default');

    const scriptPath = path.join(__dirname, '..', 'scripts', 'dng_to_ico.py');
    console.log('DNG to ICO: Executing Python script:', scriptPath);
    console.log('DNG to ICO: Input file:', inputPath);
    console.log('DNG to ICO: Output file:', outputPath);
    
    // Check if script exists
    try {
      await fs.access(scriptPath);
      console.log('DNG to ICO: Script exists');
    } catch (error) {
      console.error('DNG to ICO: Script does not exist:', scriptPath);
      return res.status(500).json({ error: 'Conversion script not found' });
    }
    ...
    python.stdout.on('data', (data: Buffer) => {
      stdout += data.toString();
      console.log('DNG to ICO stdout:', data.toString());
    });

    python.stderr.on('data', (data: Buffer) => {
      stderr += data.toString();
      console.log('DNG to ICO stderr:', data.toString());
    });

    python.on('close', async (code: number) => {
      console.log('DNG to ICO: Python script finished with code:', code);
...
// Route: DNG to ICO (Batch)
app.post('/convert/dng-to-ico/batch', uploadBatch, async (req, res) => {
  // Set CORS headers
  res.set({
    'Access-Control-Allow-Origin': '*',
    'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
  ...
  console.log('DNG->ICO batch conversion request');

  const tmpDir = path.join(os.tmpdir(), `dng-ico-batch-${Date.now()}`);
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        const scriptPath = path.join(__dirname, '..', 'scripts', 'dng_to_ico.py');
        console.log('DNG to ICO batch: Executing Python script:', scriptPath);
        console.log('DNG to ICO batch: Input file:', inputPath);
        console.log('DNG to ICO batch: Output file:', outputPath);

        try {
          await fs.access(scriptPath);
          console.log('DNG to ICO batch: Script exists');
        } catch (error) {
          console.error('DNG to ICO batch: Script does not exist:', scriptPath);
          results.push({
            originalName: file.originalname,
            outputFilename: '',
            size: 0,
        ...
        python.stdout.on('data', (data: Buffer) => {
          stdout += data.toString();
          console.log('DNG to ICO batch stdout:', data.toString());
        });

        python.stderr.on('data', (data: Buffer) => {
          stderr += data.toString();
          console.log('DNG to ICO batch stderr:', data.toString());
        });

        await new Promise<void>((resolve, reject) => {
          python.on('close', async (code: number) => {
            console.log('DNG to ICO batch: Python script finished with code:', code);
            console.log('DNG to ICO batch: stdout:', stdout);
            console.log('DNG to ICO batch: stderr:', stderr);

            try {
              if (code === 0 && await fs.access(outputPath).then(() => true).catch(() => false)) {
                const outputBuffer = await fs.readFile(outputPath);
                console.log('DNG to ICO batch: Output file size:', outputBuffer.length);
                results.push({
                  originalName: file.originalname,
                  outputFilename: path.basename(outputPath),
                  size: outputBuffer.length,
                ...
                console.error('DNG to ICO batch conversion failed. Code:', code, 'Stderr:', stderr);
                results.push({
                  originalName: file.originalname,
                  outputFilename: '',
                  size: 0,
        ...
        console.error('DNG to ICO batch conversion error for file:', file.originalname, error);
        // Error already pushed to results in the promise rejection
      }
    }
