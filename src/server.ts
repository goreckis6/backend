...
const batchFileMetadata = new Map<
  string,
  { downloadName: string; mime: string }
>();
...
const scheduleBatchFileCleanup = (storedFilename: string) => {
  ...
  const timeout = setTimeout(() => {
    fs.rm(path.join(BATCH_OUTPUT_DIR, storedFilename), { force: true }).catch(
      () => undefined
    );
...
const convertEpsFile = async (
  ...
  try {
    ...
    const qualityValue =
      quality === "high" ? 95 : quality === "medium" ? 80 : 60;
    ...
    try {
      ...
      try {
        console.log("Trying magick command with args:", [
          "convert",
          ...convertArgs,
        ]);
    ...
    const message =
      error instanceof Error ? error.message : "Unknown EPS conversion error";
    throw new Error(
      `Failed to convert EPS file: ${message}. Please ensure ImageMagick or Ghostscript is installed.`
    );
  } finally {
    await fs
...
const convertDngFile = async (
  ...
  try {
    ...
    await fs.writeFile(inputPath, file.buffer);
    console.log(
      "DNG file written successfully, size:",
      file.buffer.length,
      "bytes"
    );
    ...
    const isDng =
      ...
      fileHeader[1] === 0x49 &&
      fileHeader[2] === 0x2a &&
      fileHeader[3] === 0x00; // TIFF/DNG signature
    const isDngAlt =
      fileHeader[0] === 0x4d &&
      fileHeader[1] === 0x4d &&
      fileHeader[2] === 0x00 &&
      fileHeader[3] === 0x2a; // TIFF/DNG signature (big-endian)

    if (!isDng && !isDngAlt) {
      console.warn(
        "File does not appear to have DNG/TIFF signature, but proceeding anyway..."
      );
    } else {
    ...
    const dcrawCommands = [
      {
        args: ["-T", "-6", "-O", tiffPath, inputPath],
        desc: "dcraw -T -6 -O (with output file)",
      },
      {
        args: ["-T", "-6", inputPath],
        desc: "dcraw -T -6 (without output file)",
        cwd: tmpDir,
      },
      {
        args: ["-T", "-4", "-O", tiffPath, inputPath],
        desc: "dcraw -T -4 -O (16-bit output)",
      },
      {
        args: ["-T", "-8", "-O", tiffPath, inputPath],
        desc: "dcraw -T -8 -O (8-bit output)",
      },
      {
        args: ["-T", "-6", "-w", "-O", tiffPath, inputPath],
        desc: "dcraw -T -6 -w -O (with white balance)",
      },
    ];
    ...
    for (const cmd of dcrawCommands) {
      try {
        ...
        const dcrawResult = await execFileAsync(
          "dcraw",
          cmd.args,
          cmd.cwd ? { cwd: cmd.cwd } : {}
        );
    ...
    if (!dcrawSuccess) {
      throw new Error(
        `dcraw conversion failed with all command variants: ${
    ...
    try {
      ...
      console.error(
        "Output file not found after ImageMagick conversion:",
        statError
      );
    ...
    if (persistToDisk) {
      ...
      const result = await persistOutputBuffer(
        outputBuffer,
        downloadName,
        contentType
      );
    ...
    const message =
      error instanceof Error ? error.message : "Unknown DNG conversion error";
    console.log("=== DNG CONVERSION END (ERROR) ===");
    throw new Error(
      `Failed to convert DNG file: ${message}. Please ensure dcraw and ImageMagick are installed.`
    );
  } finally {
...
const convertDngToWebpPython = async (
  ...
  try {
    ...
    if (!scriptExists) {
      ...
      throw new Error(
        "DNG to WebP conversion script not found. Please check deployment."
      );
    }
    ...
    if (stderr.includes("Unsupported DNG file format")) {
      throw new Error(
        "Unsupported DNG file format. Please ensure the file is a valid DNG image."
      );
    }
    if (stderr.includes("DNG file I/O error")) {
      throw new Error(
        "DNG file I/O error. Please check that the file is not corrupted."
      );
    }
    if (
      stderr.includes("rawpy not available") ||
      stderr.includes("ImportError")
    ) {
      throw new Error(
        "Required Python library (rawpy) is not available. Please install it."
      );
    }
    ...
    if (!outputExists) {
      ...
      if (
        ...
        errorMessage =
          "Missing Python dependencies (rawpy or Pillow). Please install them.";
    ...
    const message =
      error instanceof Error ? error.message : `Unknown DNG->WebP error`;
...
const convertDngToIcoPython = async (
  ...
  try {
    ...
    if (!scriptExists) {
      ...
      throw new Error(
        "DNG to ICO conversion script not found. Please check deployment."
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown DNG->ICO error`;
...
const convertEpsToIcoPython = async (
  ...
  try {
    ...
    // Parse options
    const sizes = options.sizes
      ? options.sizes.split(",").map((s) => parseInt(s.trim()))
      : [16, 32, 48, 64, 128, 256];
    const quality = options.quality || "high";
    ...
    try {
      ...
      throw new Error(
        `Python ICO script execution failed: ${execError.message}`
      );
    }
    ...
    if (!outputExists) {
      console.error(
        "Python ICO script did not produce output file:",
        outputPath
      );
      console.error("Python stdout:", stdout);
      console.error("Python stderr:", stderr);
      throw new Error(
        `Python ICO script did not produce output file: ${outputPath}. Python error: ${stderr}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown EPS->ICO error`;
...
const convertEpsToWebpPython = async (
  ...
  try {
      ...
      switch (qualityValue.toLowerCase()) {
        ...
        default:
          ...
          if (
            ...
            console.warn(
              `Invalid quality value: ${qualityValue}, using default 80`
            );
            quality = 80;
          }
      }
    ...
    try {
      ...
      console.error("stderr:", stderr);
      throw new Error(
        `Python WebP script execution failed: ${execError.message}`
      );
    }
    ...
    if (!outputExists) {
      console.error(
        "Python WebP script did not produce output file:",
        outputPath
      );
      console.error("Python stdout:", stdout);
      console.error("Python stderr:", stderr);
      throw new Error(
        `Python WebP script did not produce output file: ${outputPath}. Python error: ${stderr}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown EPS->WebP error`;
...
const convertGifToIcoPython = async (
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : `Unknown GIF->ICO error`;
...
const convertCsvToJsonPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->JSON error`;
...
const convertCsvToNdjsonPython = async (
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->NDJSON error`;
...
const isRawFile = (file: Express.Multer.File) => {
  ...
  if (
    ...
    !mimetype.includes("x-png") &&
    !mimetype.includes("x-jpeg")
  ) {
    return true;
  }
...
const prepareRawBuffer = async (file: Express.Multer.File): Promise<Buffer> => {
  ...
  const inputPath = path.join(
    tmpDir,
    `input${path.extname(file.originalname) || ".raw"}`
  );
  ...
  try {
    ...
    if (!conversionSucceeded) {
      throw new Error(
        `All dcraw commands failed. Last error: ${
          lastError instanceof Error ? lastError.message : String(lastError)
        }`
      );
    }
...
const LIBREOFFICE_CONVERSIONS: Record<
  string,
  {
    convertTo: string;
    extension: string;
...
const isEpsFile = (file: Express.Multer.File) => {
  ...
  const mimetype = file.mimetype?.toLowerCase() ?? "";
  return (
    ext === "eps" || mimetype.includes("eps") || mimetype.includes("postscript")
  );
};
...
const isBmpFile = (file: Express.Multer.File) => {
  ...
  const mimetype = file.mimetype?.toLowerCase() ?? "";
  const result =
    ext === "bmp" || mimetype.includes("bmp") || mimetype.includes("image/bmp");
...
const normalizeCsvBuffer = (buffer: Buffer): NormalizedCsvResult => {
  ...
  for (const dialect of dialects) {
    ...
    if (parsed.errors && parsed.errors.length > 0) {
      console.warn(
        `CSV parse warnings (${dialect.description}):`,
...
const CALIBRE_CONVERSIONS: Record<
  string,
  {
    extension: string;
    mime: string;
...
> = {
  ...
  rtf: {
    extension: "rtf",
    mime: "application/rtf",
    intermediateExtension: "txt",
    postProcessLibreOfficeTarget: "rtf",
  },
  odt: {
    extension: "odt",
    mime: "application/vnd.oasis.opendocument.text",
    intermediateExtension: "txt",
    postProcessLibreOfficeTarget: "odt",
  },
  html: {
    extension: "html",
    mime: "text/html; charset=utf-8",
    intermediateExtension: "txt",
    postProcessLibreOfficeTarget: "html",
  },
  ...
  odp: {
    ...
    mime: "application/vnd.oasis.opendocument.presentation",
    intermediateExtension: "txt",
    postProcessPresentationTarget: "odp",
  },
  ppt: {
    extension: "ppt",
    mime: "application/vnd.ms-powerpoint",
    intermediateExtension: "txt",
    postProcessPresentationTarget: "ppt",
  },
  pptx: {
    extension: "pptx",
    mime: "application/vnd.openxmlformats-officedocument.presentationml.presentation",
    intermediateExtension: "txt",
    postProcessPresentationTarget: "pptx",
  },
  xlsx: {
    extension: "xlsx",
    mime: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    intermediateExtension: "txt",
    postProcessExcelTarget: "xlsx",
  },
  csv: {
    extension: "csv",
    mime: "text/csv; charset=utf-8",
    intermediateExtension: "txt",
    postProcessExcelTarget: "csv",
  },
  md: {
    extension: "md",
    mime: "text/markdown; charset=utf-8",
...
const execLibreOffice = async (args: string[]): Promise<CommandResult> => {
  ...
  for (const binary of LIBREOFFICE_CANDIDATES) {
    try {
      ...
      throw new Error(
        `LibreOffice execution failed using "${binary}": ${message}${stderr}${stdout}`
      );
    }
  }
  ...
  for (const binary of CALIBRE_CANDIDATES) {
    try {
      ...
      if (error?.code === "TIMEOUT" || error?.signal === "SIGTERM") {
        console.log(
          `Calibre conversion timed out after 5 minutes for binary: ${binary}`
        );
        lastError = new Error(
          `Calibre conversion timed out after 5 minutes. Large files may need more time to process.`
        );
        continue;
      }
      ...
      throw new Error(
        `Calibre execution failed using "${binary}": ${message}${stderr}${stdout}`
      );
    }
  }
...
const convertCsvDirectlyToMarkdown = async (
  ...
  try {
    ...
    throw new Error(
      `Failed to convert CSV to Markdown: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
  }
      ...
      .each((rowIndex, row) => {
        ...
        $(row)
          ...
          .each((cellIndex, cell) => {
            ...
            if (
              cellText === "" ||
              tableRows.some((existingRow) => existingRow.includes(cellText))
            ) {
              const rowspan = parseInt($(cell).attr("rowspan") || "1", 10);
              if (rowspan > 1) {
                for (let i = 1; i < rowspan; i++) {
                  const nextRow = tableRows[rowIndex + i]
                    ? tableRows[rowIndex + i].split(",")
                    : [];
            ...
            const colspan = colspanAttr
              ? Math.max(parseInt(colspanAttr, 10) || 1, 1)
              : 1;
...
const convertDocWithLibreOffice = async (
  ...
  try {
    ...
    if (inputExtension !== ".docx") {
      ...
      const generatedDocx = files.find((name) =>
...
const convertCsvToDocxFallback = async (
  ...
  const tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "morphy-csv-docx-fallback-")
  );
  const originalBase = path.basename(
    file.originalname,
    path.extname(file.originalname)
  ...
  try {
    ...
    const { stdout: odtStdout, stderr: odtStderr } = await execLibreOffice(
      csvToOdtArgs
    );
    ...
    for (const args of commandVariants) {
      try {
        ...
        docxFile =
          finalFiles.find((name) => name.toLowerCase().endsWith(".docx")) ||
          null;
        ...
        if (docxFile) {
          ...
          if (
            ...
            console.warn(
              "Invalid DOCX file created from ODT, trying next variant..."
            );
            outputBuffer = null;
            docxFile = null;
          }
    ...
    if (!docxFile || !outputBuffer) {
      throw new Error(
        `LibreOffice did not produce a valid DOCX file. Last error: ${
          lastError instanceof Error ? lastError.message : String(lastError)
        }`
      );
    }
...
const convertCsvToDocxEnhanced = async (
  ...
  try {
    ...
    for (const libreOfficeArgs of libreOfficeVariants) {
      try {
        console.log(
          "Running LibreOffice HTML to DOCX conversion:",
          libreOfficeArgs
        );
        ...
        const files = await fs.readdir(tmpDir);
        docxFile =
          files.find((name) => name.toLowerCase().endsWith(".docx")) || null;
    ...
    if (!docxFile) {
      throw new Error(
        `LibreOffice did not produce a DOCX file. Last error: ${
          lastError instanceof Error ? lastError.message : String(lastError)
        }`
      );
    }
...
const createHtmlTableFromCsv = (csvData: string[][]): string => {
  ...
  let html =
    '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>CSV Data</title>';
  html +=
    "<style>body{font-family:Arial,sans-serif;margin:20px;}table{border-collapse:collapse;width:100%;}th,td{border:1px solid #ddd;padding:8px;text-align:left;}th{background-color:#f2f2f2;font-weight:bold;}</style>";
...
const convertCsvToEpubCustom = async (
  ...
  try {
    ...
    const rows: string[][] =
      parsed && Array.isArray((parsed as any).data)
        ? ((parsed as any).data as unknown as string[][]).map((r: unknown) =>
            Array.isArray(r) ? (r as string[]) : [String(r ?? "")]
          )
        : [];
...
<body>
    ...
    console.log("CSV->EPUB (HTML) conversion successful:", {
      filename: downloadName,
      size: outputBuffer.length,
    });
...
<head>
    ...
    <meta name="generator" content="MorphyHub Converter" />
    <style>
        body { 
...
<body>
    <div class="metadata">
        ...
        <p><strong>Generated by:</strong> MorphyHub Converter</p>
    </div>
    ${htmlContent
      .replace("<h1>", "")
      .replace("</h1>", "")
      .replace("<p><strong>Author:</strong>", "")
      .replace("</p>", "")}
</body>
</html>`;
...
const createSimpleHtmlFromCsv = async (
  ...
  try {
    ...
    const rows: string[][] =
      parsed && Array.isArray((parsed as any).data)
        ? ((parsed as any).data as unknown as string[][])
            .map((r: unknown) => {
              if (Array.isArray(r)) {
...
const convertCsvToMobiSimple = async (
  ...
  const tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "morphy-csv-mobi-simple-")
  );
  const originalBase = path.basename(
    file.originalname,
    path.extname(file.originalname)
  ...
  try {
    ...
    const htmlContent = await createSimpleHtmlFromCsv(
      file,
      options.title || sanitizedBase,
    ...
    console.log(
      `Generated HTML: ${htmlPath} (${htmlContent.length} characters)`
    );
    ...
    console.log(
      `Starting Calibre conversion for file: ${file.originalname} (${file.buffer.length} bytes)`
    );
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->MOBI error`;
...
const convertCsvToMobiOptimized = async (
  ...
  const tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "morphy-csv-mobi-opt-")
  );
  const originalBase = path.basename(
    file.originalname,
    path.extname(file.originalname)
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Optimized Python script did not produce output file: ${outputPath}`
      );
    }
...
const convertCsvToOdtPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python ODT script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->ODT error`;
...
const convertCsvToPdfPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python PDF script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->PDF error`;
...
const convertCsvToDocxPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python DOCX script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->DOCX error`;
    throw new Error(`Failed to convert CSV to DOCX: ${message}`);
  } finally {
    await fs
      .rm(tmpDir, { recursive: true, force: true })
...
const convertCsvToPptPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python PPT script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->PPT error`;
...
const convertCsvToPptxPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python PPTX script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->PPTX error`;
...
const convertCsvToRtfPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python RTF script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->RTF error`;
...
const convertCsvToTxtPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python TXT script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->TXT error`;
...
const convertCsvToXlsPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python XLS script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->XLS error`;
...
const convertCsvToXlsxPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python XLSX script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->XLSX error`;
...
const convertCsvToEbookPython = async (
  ...
  try {
    ...
    console.log(
      "Running Python CSV to e-book converter with args:",
      pythonArgs
    );
    ...
    try {
      ...
      if (targetFormat === "mobi") {
        const htmlPath = path.join(tmpDir, `${safeBase}.html`);
        const fallbackHtml = await createSimpleHtmlFromCsv(
          file,
          options.title || sanitizedBase,
          options.author || "Unknown"
        );
        ...
        const fallbackContent = await createSimpleHtmlFromCsv(
          file,
          options.title || sanitizedBase,
          options.author || "Unknown"
        );
    ...
    console.error(
      `CSV->${targetFormat.toUpperCase()} conversion error:`,
      error
    );
    ...
    const message =
      error instanceof Error
        ? error.message
        : `Unknown CSV->${targetFormat.toUpperCase()} error`;
    throw new Error(
      `Failed to convert CSV to ${targetFormat.toUpperCase()}: ${message}`
    );
  } finally {
    await fs
      .rm(tmpDir, { recursive: true, force: true })
...
const convertCsvToHtmlPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python HTML script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->HTML error`;
...
const convertCsvToDocxWithDocxLib = async (
  ...
  try {
    ...
    if (parsed && Array.isArray(parsed.errors) && parsed.errors.length) {
      console.warn(
        "CSV parse warnings:",
        parsed.errors.map((e: any) => ({ message: e.message, row: e.row }))
      );
    }
    ...
    const tableRows: TableRow[] = safeRows.map(
      (row) =>
        new TableRow({
          children: row.map(
            (cell) =>
    ...
    if (
      ...
      console.warn(
        "Generated DOCX does not start with PK header, but proceeding. Size:",
        outputBuffer.length
      );
    }
...
const convertCsvWithLibreOffice = async (
  ...
  if (targetFormat === "md") {
    console.log(
      "Using direct CSV to Markdown conversion (bypassing LibreOffice)"
    );
  ...
  if (targetFormat === "docx") {
    console.log(
      "Using docx library for CSV to DOCX conversion (avoiding LibreOffice)"
    );
  ...
  const findOutputFile = async (): Promise<string | null> => {
    ...
    const directMatch = files.find(
      (name) => name.toLowerCase() === `${safeBase.toLowerCase()}${targetExt}`
    );
    if (directMatch) {
      return path.join(tmpDir, directMatch);
    }

    const fallbackMatch = files.find((name) =>
      ...
      try {
        ...
        if (!outputPath) {
          ...
          try {
            ...
            console.log(
              "Looking for files with extension:",
          ...
          throw new Error(
            `LibreOffice did not produce an output file for args: ${args.join(
              " "
            )}`
          );
        }
        ...
        const outputBuffer = await fs.readFile(outputPath);
        const downloadName = `${sanitizeFilename(originalBase)}.${
          conversion.extension
        }`;
...
const convertWithCalibre = async (
  ...
  console.log(
    `Calibre conversion requested: ${file.originalname} -> ${targetFormat}`
  );
  console.log(
    `Available Calibre conversions:`,
    Object.keys(CALIBRE_CONVERSIONS)
  );
  ...
  const sanitizedBase = sanitizeFilename(originalBase);
  const safeBase = `${sanitizedBase}_${randomUUID()}`;
  const intermediateExtension =
    conversion.intermediateExtension ?? conversion.extension;
  try {
    const inputPath = path.join(
      tmpDir,
      `${safeBase}${path.extname(file.originalname) || ".epub"}`
    );
    await fs.writeFile(inputPath, file.buffer);
    const outputPath = path.join(
      tmpDir,
      `${safeBase}.${intermediateExtension}`
    );
    ...
    try {
      ...
      const outputBuffer = await fs.readFile(outputPath);
      console.log(
        `Calibre conversion successful, output size: ${outputBuffer.length} bytes`
      );
    } catch (calibreError) {
      ...
      if (targetFormat === "html") {
        ...
        const { stdout: fallbackStdout, stderr: fallbackStderr } =
          await execCalibre(fallbackArgs);
    ...
    if (conversion.postProcessPresentationTarget) {
      console.log(
        `Post-processing presentation: ${intermediateExtension} -> ${conversion.postProcessPresentationTarget}, persistToDisk: ${persistToDisk}`
      );
      ...
      try {
        ...
        console.log(
          `Presentation post-process result: filename=${
            result.filename
          }, hasStoredFilename=${!!result.storedFilename}, bufferSize=${
        ...
        console.error(
          "Presentation post-processing failed:",
          presentationError
        );
    ...
    if (conversion.postProcessLibreOfficeTarget) {
      console.log(
        `Post-processing with LibreOffice: ${intermediateExtension} -> ${conversion.postProcessLibreOfficeTarget}, persistToDisk: ${persistToDisk}`
      );
      console.log(`Input buffer size: ${outputBuffer.length} bytes`);
      try {
        ...
        console.log(
          `LibreOffice post-process result: filename=${
            result.filename
          }, hasStoredFilename=${!!result.storedFilename}, bufferSize=${
            result.buffer.length
          }`
        ...
        throw new Error(
          `LibreOffice post-processing failed: ${
            libreOfficeError instanceof Error
              ? libreOfficeError.message
              : String(libreOfficeError)
          }`
        );
      }
    }
    ...
    const message =
      error instanceof Error ? error.message : "Unknown Calibre error";
    throw new Error(
      `Failed to convert ${file.originalname} (${targetFormat}): ${message}`
    );
  } finally {
    await fs
      .rm(tmpDir, { recursive: true, force: true })
...
const convertBufferWithLibreOffice = async (
  ...
  const sanitizedBase = sanitizeFilename(originalBase);
  const safeBase = `${sanitizedBase}_${randomUUID()}`;
  const normalizedExtension = inputExtension.startsWith(".")
    ? inputExtension
    : `.${inputExtension}`;
  ...
  try {
    ...
    if (
      ["odp", "ppt", "pptx"].includes(targetFormat) &&
    ...
    if (!outputName) {
      throw new Error(
        `LibreOffice did not produce an output file for post-processing to ${conversion.extension}`
      );
    }
...
const convertTxtToPresentation = async (
  ...
  const sanitizedBase = sanitizeFilename(originalBase);
  const tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "morphy-presentation-")
  );
  ...
  try {
    ...
    const filterMap2: Record<"odp" | "ppt", string> = {
      odp: "impress8",
    ...
    const variants2: string[][] = [
      [
        ...
        "--nofirststartwizard",
        "--convert-to",
        `${targetFormat}:${filterMap2[targetFormat]}`,
        "--outdir",
        tmpDir,
        pptxPath,
      ],
    ...
    for (const variant of variants2) {
      ...
      try {
        ...
        console.warn(
          "LibreOffice PPTX conversion attempt failed, trying fallback...",
          e
        );
      }
    }
    if (!ok)
      throw new Error(
        "LibreOffice failed converting PPTX to presentation format"
      );
    if (stdout.trim())
    ...
    const outputFile = files.find((f) =>
      f.toLowerCase().endsWith(expectedExt[targetFormat])
    );
    if (!outputFile)
      throw new Error(
        `LibreOffice did not produce ${targetFormat} output file. Available files: ${files.join(
          ", "
        )}`
      );
    ...
    console.log(
      `Presentation conversion successful, output size: ${outputBuffer.length} bytes`
    );
    ...
    if (persistToDisk) {
      ...
      console.log(
        "Persisting presentation file to disk with 10-minute cleanup..."
      );
      const result = await persistOutputBuffer(
        outputBuffer,
        downloadName,
      ...
      if (result.storedFilename) {
        ...
        setTimeout(async () => {
          try {
            ...
            console.log(
              `Cleaned up presentation file: ${result.storedFilename}`
            );
            batchFileMetadata.delete(result.storedFilename!);
          } catch (cleanupError) {
            console.warn(
              `Cleanup failed for presentation file ${result.storedFilename}:`,
              cleanupError
            );
          }
...
const buildSlidesFromText = (
  ...
  for (const raw of lines) {
    ...
    const isHeading =
      (line.length < 60 && !/[.,;:]$/.test(line)) || /^chapter\s+/i.test(line);
...
const buildCalibreArgs = (
  ...
  if (options.githubCompatible === "true")
    args.push("--transform-css", "github");
  if (options.kindleOptimized === "true") args.push("--mobi-file-type", "both");
  if (options.openSourceCompatible === "true")
    args.push("--prefer-metadata-author-sort");
...
const buildLibreOfficeFilterArgs = (
  ...
  const delimiter =
    options.delimiter === "\t" ? "9" : options.delimiter === ";" ? "59" : "44";
...
const convertDocxWithLibreOffice = async (
  ...
  const safeBase = `${sanitizedBase}_${randomUUID()}`;
  const inputExt =
    inputHint === "docx" ? ".docx" : inputHint === "odt" ? ".odt" : ".doc";
  ...
  try {
    ...
    for (const exportArgs of exportVariants) {
      try {
        ...
        const files = await fs.readdir(tmpDir);
        const epubFile = files.find((name) =>
          name.toLowerCase().endsWith(".epub")
        );
        if (!epubFile) {
          console.warn(
            "EPUB file not found after LibreOffice run, trying next variant..."
          );
          continue;
        }
        ...
        console.warn(
          "LibreOffice DOCX->EPUB command failed, trying fallback...",
          variantError
        );
        continue;
      }
    ...
    const message =
      error instanceof Error ? error.message : "Unknown LibreOffice EPUB error";
...
const convertCsvToMdPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python Markdown script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->Markdown error`;
  ...
  try {
    ...
    const { stdout, stderr } = await execFileAsync(
      ...
      [
        ...
        outputPath,
        "--title",
        options.bookTitle || sanitizedBase,
        "--author",
        options.author || "CSV Converter",
        "--chunk-size",
        chunkSize.toString(),
      ].concat(options.includeHeaders === "false" ? ["--no-headers"] : []),
      {
    ...
    if (!outputExists) {
      throw new Error(
        `Python MOBI script did not produce output file: ${outputPath}`
      );
    }
...
const convertCsvToOdpPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python ODP script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->ODP error`;
...
app.use((req, res, next) => {
  ...
  console.log(
    `Request received: ${req.method} ${req.path}, Content-Length: ${contentLength} bytes`
  );
    ...
    try {
      ...
      if (!file) {
        ...
        if (req.files && Array.isArray(req.files) && req.files.length > 0) {
          ...
          console.log(
            "Found files in object format, checking for common field names"
          );
        ...
        if (file.mimetype) {
          ...
          if (file.mimetype === "application/octet-stream") {
            ...
            if (file.buffer && file.buffer.length > 0) {
              ...
              else if (file.buffer[0] === 0x49 && file.buffer[1] === 0x49) {
                // II (Intel byte order)
                ext = "dng";
                console.log("Detected DNG file from signature");
              } else if (file.buffer[0] === 0x4d && file.buffer[1] === 0x4d) {
      ...
      if (!file.buffer || file.buffer.length === 0) {
        ...
        return res
          .status(400)
          .json({ error: "Invalid file upload - empty file" });
      }
      ...
      if (file.buffer && file.buffer.length > 10) {
        console.log(
          "First 10 bytes:",
          Array.from(file.buffer.slice(0, 10))
            .map((b: number) => "0x" + b.toString(16).padStart(2, "0"))
            .join(" ")
        );
        console.log(
          "First 2 bytes check - [0]:",
          file.buffer[0],
          "expected: 0x42 (66)"
        );
        console.log(
          "First 2 bytes check - [1]:",
          file.buffer[1],
          "expected: 0x4D (77)"
        );
        console.log(
          "Is BMP signature?:",
          file.buffer[0] === 0x42 && file.buffer[1] === 0x4d
        );
      }
      ...
      if (targetFormat === "webp" && file.buffer && file.buffer.length > 2) {
        ...
        const isDngSignature =
          (file.buffer[0] === 0x49 && file.buffer[1] === 0x49) || // II (Intel)
        ...
        if (isDngSignature) {
          ...
          try {
            const result = await convertDngToWebpPython(
              file,
              requestOptions,
              true
            );
            ...
            console.log(
              "=== CONVERSION REQUEST END (SUCCESS via emergency DNG entry) ==="
            );
            return;
          } catch (emergencyError) {
            console.error(
              "!!! Emergency DNG to WebP conversion failed:",
              emergencyError
            );
            console.error("Error details:", {
              message:
      ...
      if (!isBMP && file.buffer && file.buffer.length > 2) {
        if (file.buffer[0] === 0x42 && file.buffer[1] === 0x4d) {
          // BM signature
          console.log(
            "!!! BMP signature detected in buffer, forcing isBMP to true !!!"
          );
          isBMP = true;
        }
      }
      ...
      if (targetFormat === "ico") {
        console.log(
          "!!! ICO conversion requested - checking if this should be BMP to ICO !!!"
        );
      ...
      console.log(
        "Available CALIBRE_CONVERSIONS:",
        Object.keys(CALIBRE_CONVERSIONS)
      );
      console.log(
        "Target format in CALIBRE_CONVERSIONS?",
        !!CALIBRE_CONVERSIONS[targetFormat]
      );
      ...
      if (
        !result &&
        (isDocFile(file) || isDocxFile(file) || isOdtFile(file)) &&
        targetFormat === "epub"
      ) {
        console.log(
          "Single: Routing to LibreOffice (DOC/DOCX/ODT to EPUB conversion)"
        );
        const inputHint = isDocxFile(file)
          ? "docx"
          : isOdtFile(file)
          ? "odt"
          : "doc";
        result = await convertDocxWithLibreOffice(
          file,
          inputHint,
          requestOptions,
          true
      ...
      if (!result && isEPUB && CALIBRE_CONVERSIONS[targetFormat]) {
        ...
        result = await convertWithCalibre(
          file,
          targetFormat,
          requestOptions,
          true
        ...
        console.log(
          `Single: Routing to Python (CSV to ${targetFormat.toUpperCase()} conversion)`
        );
        result = await convertCsvToEbookPython(
          file,
          targetFormat,
          requestOptions,
          true
        ...
        !result &&
        isCSV &&
        LIBREOFFICE_CONVERSIONS[targetFormat] &&
        targetFormat !== "doc"
      ) {
        console.log("Single: Routing to LibreOffice (CSV conversion)");
        result = await convertCsvWithLibreOffice(
          file,
          targetFormat,
          requestOptions,
          true
        ...
        !result &&
        (isDOC || file.originalname.toLowerCase().endsWith(".doc")) &&
        targetFormat === "csv"
      ) {
        console.log("Single: Routing to LibreOffice (DOC to CSV conversion)");
        result = await convertDocWithLibreOffice(
          file,
          "doc-to-csv",
          requestOptions,
          true
        ...
        !result &&
        isEPS &&
        ["webp", "png", "jpeg", "jpg", "ico"].includes(targetFormat)
      ) {
        ...
        !result &&
        isDNG &&
        ["png", "jpeg", "jpg"].includes(targetFormat)
      ) {
        ...
        if (
          ...
          console.log(
            "Falling back to Sharp image processing - this should not happen for DOC files!"
          );
          console.log("File details:", {
            originalname: file.originalname,
            mimetype: file.mimetype,
          ...
          try {
            ...
            console.log(
              `Metadata => ${metadata.width}x${metadata.height}, format: ${metadata.format}`
            );
          } catch (sharpError) {
            ...
            if (
              isDNG ||
              (file.originalname &&
                file.originalname.toLowerCase().endsWith(".dng"))
            ) {
              throw new Error(
                "DNG files cannot be processed directly by Sharp. Please use the Python conversion method."
              );
            }
            ...
            if (
              ...
              throw new Error(
                "BMP files cannot be processed directly by Sharp. Please use ImageMagick or another conversion method."
              );
            }
            ...
            throw new Error(
              `Unsupported image format: ${filename}. Sharp cannot process this file type.`
            );
          }
          ...
          switch (targetFormat) {
            ...
            default:
              return res
                .status(400)
                .json({ error: "Unsupported output format" });
          }
          ...
          const outputName = `${file.originalname.replace(
            /\.[^.]+$/,
            ""
          )}.${fileExtension}`;

          result = await persistOutputBuffer(
            outputBuffer,
            outputName,
            contentType
          );
      ...
      if (error instanceof Error && error.message.includes("ENOENT")) {
        console.error(
          "ENOENT error - likely missing binary or file path issue"
        );
      }
  ...
  async (req, res) => {
    ...
    for (const file of files) {
      try {
        ...
        console.log(
          `Processing ${file.originalname}: isCSV=${isCSV}, isEPUB=${isEPUB}, isEPS=${isEPS}, isDNG=${isDNG}, isDOC=${isDOC}, isBMP=${isBMP}, format=${format}, mimetype=${file.mimetype}`
        );
        ...
        if (
          ...
          console.log(
            "Batch: Routing to LibreOffice (DOC/DOCX/ODT to EPUB conversion)"
          );
          const inputHint = isDocxFile(file)
            ? "docx"
            : isOdtFile(file)
            ? "odt"
            : "doc";
          output = await convertDocxWithLibreOffice(
            file,
            inputHint,
            requestOptions,
            true
          ...
          console.log(
            `Batch: Routing to Python (CSV to ${format.toUpperCase()} conversion)`
          );
          output = await convertCsvToEbookPython(
            file,
            format,
            requestOptions,
            true
          ...
          isCSV &&
          LIBREOFFICE_CONVERSIONS[format] &&
          format !== "doc" &&
          format !== "docx"
        ) {
          console.log("Routing to LibreOffice (CSV conversion)");
          output = await convertCsvWithLibreOffice(
            file,
            format,
            requestOptions,
            true
          ...
          output = await convertDocWithLibreOffice(
            file,
            "doc-to-csv",
            requestOptions,
            true
          ...
          isEPS &&
          ["webp", "png", "jpeg", "jpg", "ico"].includes(format)
        ) {
          ...
          throw new Error(
            `Unsupported input file type or target format for batch conversion. File: ${file.originalname}, isCSV: ${isCSV}, isEPUB: ${isEPUB}, isEPS: ${isEPS}, isDNG: ${isDNG}, isDOC: ${isDOC}, isBMP: ${isBMP}, format: ${format}`
          );
        }

        console.log(
          `Batch result for ${file.originalname}: filename=${
            output.filename
          }, hasStoredFilename=${!!output.storedFilename}, bufferSize=${
        ...
        results.push({
          ...
          success: true,
          downloadPath: output.storedFilename
            ? `/download/${encodeURIComponent(output.storedFilename)}`
            : undefined,
        ...
        console.error(
          `Batch conversion failed for ${file.originalname}:`,
          error
        );
        results.push({
          originalName: file.originalname,
...
app.get("/download/:filename", async (req, res) => {
  try {
    ...
    console.log("Download request:", {
      ...
      metadata: metadata
        ? { downloadName: metadata.downloadName, mime: metadata.mime }
        : null,
    });
...
app.get("/test-python", async (req, res) => {
  try {
    ...
    const execFileAsync = promisify(execFile);
    const { stdout, stderr } = await execFileAsync("/opt/venv/bin/python3", [
      "--version",
    ]);
    res.json({
      success: true,
...
app.post("/api/preview/docx", upload.single("file"), async (req, res) => {
  ...
  const tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "morphy-docx-preview-")
  );
  try {
    const file = req.file;
    if (!file) {
    ...
    const outputPath = path.join(
      tmpDir,
      file.originalname.replace(/\.(docx)$/i, ".html")
    );
    await fs.writeFile(inputPath, file.buffer);
...
app.post("/api/preview/rtf", upload.single("file"), async (req, res) => {
  ...
  const tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "morphy-rtf-preview-")
  );
  try {
    const file = req.file;
    if (!file) {
    ...
    const outputPath = path.join(
      tmpDir,
      file.originalname.replace(/\.(rtf)$/i, ".html")
    );
    await fs.writeFile(inputPath, file.buffer);
    ...
    try {
      ...
      // Wrap HTML in styled template with toolbar (same as PDF viewer)
      const styledHtml = `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
          ...
          <script>
            ...
            function previousPage() {
              // RTF is single page, so this is disabled
            }
...
app.post("/api/preview/heic", upload.single("file"), async (req, res) => {
  ...
  const tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "morphy-heic-preview-")
  );
  ...
  try {
    ...
    const args = [scriptPath, heicPath, pngPath, "--max-dimension", "2048"];
    ...
    const message =
      error instanceof Error ? error.message : "Unknown HEIC preview error";
    res.set({
      "Access-Control-Allow-Origin": "*",
      "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
    ...
    res
      .status(500)
      .json({ error: `Failed to generate HEIC preview: ${message}` });
  } finally {
    await fs
      .rm(tmpDir, { recursive: true, force: true })
      .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "HEIC to PNG: Failed to create temp directory:",
          mkdirError
        );
        res.set({
          "Access-Control-Allow-Origin": "*",
        ...
        try {
          if (!res.headersSent) {
            if (
              ...
              console.error(
                "HEIC to PNG conversion failed. Code:",
                code,
                "Stderr:",
                stderr
              );
...
// Route: HEIC to JPG (Single)
app.post(
  "/convert/heic-to-jpg/single",
  upload.single("file"),
  async (req, res) => {
    // Set CORS headers
    res.set({
    ...
    console.log("HEIC->JPG single conversion request");

    const tmpDir = path.join(os.tmpdir(), `heic-jpg-${Date.now()}`);
    ...
    try {
      ...
      try {
        ...
        console.error(
          "HEIC to JPG: Failed to create temp directory:",
          mkdirError
        );
        res.set({
          "Access-Control-Allow-Origin": "*",
      ...
      try {
        ...
        console.error("HEIC to JPG: Failed to write input file:", writeError);
        res.set({
          "Access-Control-Allow-Origin": "*",
          "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
          "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
      ...
      console.log("HEIC to JPG: Executing Python script:", scriptPath);
      console.log("HEIC to JPG: Input file:", inputPath);
      console.log("HEIC to JPG: Output file:", outputPath);

      try {
        await fs.access(scriptPath);
        console.log("HEIC to JPG: Script exists");
      } catch (error) {
        console.error("HEIC to JPG: Script does not exist:", scriptPath);
        res.set({
          "Access-Control-Allow-Origin": "*",
          "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
          "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
      ...
      // Optional quality param (0-100) for JPG compression
      ...
      python.on("error", async (error: Error) => {
        console.error("HEIC to JPG: Failed to start Python process:", error);
        if (!res.headersSent) {
          res.set({
            "Access-Control-Allow-Origin": "*",
            "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      ...
      python.stdout.on("data", (data: Buffer) => {
        stdout += data.toString();
        console.log("HEIC to JPG stdout:", data.toString());
      });

      python.stderr.on("data", (data: Buffer) => {
        stderr += data.toString();
        console.log("HEIC to JPG stderr:", data.toString());
      });

      const timeout = setTimeout(async () => {
        console.error("HEIC to JPG: Conversion timeout after 5 minutes");
        python.kill();
        if (!res.headersSent) {
          res.set({
            "Access-Control-Allow-Origin": "*",
      ...
      python.on("close", async (code: number) => {
        clearTimeout(timeout);
        console.log("HEIC to JPG: Python script finished with code:", code);
        console.log("HEIC to JPG: stdout:", stdout);
        console.log("HEIC to JPG: stderr:", stderr);
        ...
        try {
          if (!res.headersSent) {
            if (
              ...
              console.error(
                "HEIC to JPG conversion failed. Code:",
                code,
                "Stderr:",
                stderr
              );
          ...
          console.error("Error handling conversion result (JPG):", error);
          if (!res.headersSent) {
            res.set({
              "Access-Control-Allow-Origin": "*",
              "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
...
// Route: HEIC to JPG (Batch) - OPTIONS for CORS preflight
app.options("/convert/heic-to-jpg/batch", (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
...
// X3F Preview endpoint - generates JPEG preview using rawpy (via viewers/x3f_to_jpeg.py)
app.post("/api/preview/x3f", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const tmpDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "morphy-x3f-preview-")
    );
    const inputPath = path.join(tmpDir, file.originalname);
    const outputPath = inputPath.replace(/\.(x3f)$/i, ".jpg");
    await fs.writeFile(inputPath, file.buffer);

    const scriptPath = path.join(__dirname, "..", "viewers", "x3f_to_jpeg.py");
    ...
    try {
      await fs.access(scriptPath);
      await execFileAsync("/opt/venv/bin/python", [
        scriptPath,
        inputPath,
        outputPath,
        "--max-dimension",
...
app.post("/api/preview/nef", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const tmpDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "morphy-nef-preview-")
    );
    const inputPath = path.join(tmpDir, file.originalname);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        const result = await execFileAsync(pythonPath, args, {
          timeout: 120000,
        }); // 2 min timeout for RAW processing
        stdout = result.stdout;
        stderr = result.stderr;
      } catch (execError: any) {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "PEF execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown PEF preview error";
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
        "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate PEF preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.(heic|heif)$/i, ".eps")
      );
      await fs.writeFile(inputPath, file.buffer);
            ...
            try {
              if (
                ...
                results.push({
                  ...
                  size: outputBuffer.length,
                  success: true,
                  downloadPath: `data:application/postscript;base64,${outputBuffer.toString(
                    "base64"
                  )}`,
                });
              } else {
...
app.post("/api/preview/tiff", upload.single("file"), async (req, res) => {
  ...
  const tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "morphy-tiff-preview-")
  );
  ...
  try {
    ...
    const args = [scriptPath, tiffPath, pngPath, "--max-dimension", "2048"];
    ...
    const message =
      error instanceof Error ? error.message : "Unknown TIFF preview error";
    res
      .status(500)
      .json({ error: `Failed to generate TIFF preview: ${message}` });
  } finally {
    await fs
      .rm(tmpDir, { recursive: true, force: true })
      .catch(() => undefined);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const args = [scriptPath, docxPath, htmlPath];
      ...
      if (!htmlExists) {
        throw new Error(
          `Python script did not produce HTML preview: ${htmlPath}`
        );
      }
      ...
      const splitTable = (
        ...
        let rowsContent = tbodyMatch
          ? tbodyMatch[1]
          : tableContent.replace(/<thead[^>]*>[\s\S]*?<\/thead>/i, "");
        ...
        const colCount = firstRowMatch
          ? (firstRowMatch[1].match(/<(th|td)[^>]*>/gi) || []).length
          : 3;
        ...
        const actualMaxRows =
          isWideTable && rows.length <= maxRowsPerPage
            ? Math.max(3, Math.floor(rows.length / 2))
            : maxRowsPerPage;
        ...
        for (let i = 0; i < rows.length; i += actualMaxRows) {
          ...
          const header =
            thead || (headerRow ? `<thead>${headerRow}</thead>` : "");
      ...
      const splitIntoPages = (content: string): string[] => {
        ...
        while ((tableMatch = tableRegex.exec(content)) !== null) {
          ...
          if (shouldSplit || rowCount > 3) {
            // Split if more than 3 rows
            ...
            console.log(
              `Splitting table: ${rowCount} rows, ${colCount} columns, ${rowsPerPage} rows per page`
            );
        ...
        for (let i = 0; i <= tables.length; i++) {
          const startIdx =
            i === 0 ? 0 : tables[i - 1].index + tables[i - 1].content.length;
          ...
          const blockTagPattern =
            /<(p|div|h[1-6]|ul|ol|blockquote|pre|hr|section|article|header|footer|nav|aside)[^>]*>[\s\S]*?<\/\1>/gi;
          let match;
          let segLastIndex = 0;

          while ((match = blockTagPattern.exec(segment)) !== null) {
            if (match.index > segLastIndex) {
              const textBefore = segment
                .substring(segLastIndex, match.index)
                .trim();
          ...
          while ((tableMatch = tableRegex.exec(content)) !== null) {
            if (tableMatch.index > tableLastIndex) {
              const beforeTable = content
                .substring(tableLastIndex, tableMatch.index)
                .trim();
              if (beforeTable) {
                // Split non-table content by paragraphs
                const paras = beforeTable
                  .split(/(<p[^>]*>[\s\S]*?<\/p>)/gi)
                  .filter((p) => p.trim());
        ...
        if (blockElements.length === 0) {
          ...
          while ((paraMatch = paraRegex.exec(content)) !== null) {
            ...
            if (
              currentPageLength + para.length > charsPerPage &&
        ...
        const totalChars = blockElements.reduce(
        ...
        const estimatedPages = Math.max(
          1,
          Math.ceil(totalChars / charsPerPage)
        );
        ...
        for (const element of blockElements) {
          ...
          // If adding this element would exceed page size and we already have content, start new page
            ...
            currentPageSize + elementSize > charsPerPage &&
            currentPage.length > 0
          ) {
            pages.push(currentPage.join("\n"));
            currentPage = [element];
      ...
      pages.forEach((page, idx) => {
        console.log(
          `Page ${idx + 1}: ${page.length} characters, ${
            (page.match(/<tr[^>]*>/gi) || []).length
          } table rows`
        );
      });
      ...
      const pagesHtml = pages
        .map(
          (pageContent, index) =>
            `<div class="docx-a4-page" data-page="${
              index + 1
            }">${pageContent}</div>`
        )
        .join("\n");
      ...
      const message =
        error instanceof Error ? error.message : "Unknown DOCX preview error";
      res
        .status(500)
        .json({ error: `Failed to generate DOCX preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const args = [scriptPath, rtfPath, htmlPath];
    ...
    `;
      ...
      const message =
        error instanceof Error ? error.message : "Unknown RTF preview error";
      res
        .status(500)
        .json({ error: `Failed to generate RTF preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const args = [scriptPath, odtPath, htmlPath];
    ...
    `;
      ...
      const message =
        error instanceof Error ? error.message : "Unknown ODT preview error";
      ...
      res
        .status(500)
        .json({ error: `Failed to generate ODT preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const args = [scriptPath, pdfPath, outputPath];
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "PDF execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown PDF preview error";
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
        "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate PDF preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
// TXT Preview endpoint - OPTIONS for CORS preflight
app.options("/api/preview/txt", (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!scriptExists) {
        console.error(`Python script not found: ${scriptPath}`);
        res.status(500).json({ error: "TXT preview script not found" });
        return;
      }
      ...
      <body>
        ...
        <script>
          ...
          function updateZoom() {
            textContainer.style.transform = \`scale(\${currentZoom / 100})\`;
            textContainer.style.transformOrigin = 'top center';
            document.getElementById('zoom-level').textContent = \`\${currentZoom}%\`;
          }
          ...
          function fitWidth() {
            ...
            const contentWidth = textContainer.scrollWidth || 1200;
          ...
          function previousPage() {
            // TXT is single page, so this is disabled
          }
    ...
    `;
      ...
      const message =
        error instanceof Error ? error.message : "Unknown TXT preview error";
      // Ensure CORS headers are set even on error
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate TXT preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post("/api/preview/md", uploadDocument.single("file"), async (req, res) => {
  ...
  try {
    ...
    const args = [scriptPath, mdPath, htmlPath];
    ...
    const message =
      error instanceof Error ? error.message : "Unknown Markdown preview error";
    // Ensure CORS headers are set even on error
    res.set({
      "Access-Control-Allow-Origin": "*",
      "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    ...
    res
      .status(500)
      .json({ error: `Failed to generate Markdown preview: ${message}` });
  } finally {
    await fs
      .rm(tmpDir, { recursive: true, force: true })
      .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "RAF execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown RAF preview error";
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
        "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate RAF preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "ORF execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown ORF preview error";
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
        "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate ORF preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "DNG execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown DNG preview error";
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
        "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate DNG preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "ARW execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown ARW preview error";
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
        "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate ARW preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "X3F execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown X3F preview error";
      res
        .status(500)
        .json({ error: `Failed to generate X3F preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "DCR execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown DCR preview error";
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
        "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate DCR preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "CR2 execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown CR2 preview error";
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
        "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate CR2 preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "NEF execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown NEF preview error";
      res
        .status(500)
        .json({ error: `Failed to generate NEF preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const args = [scriptPath, pyPath, outputPath, "--max-size-mb", "10"];
      ...
      htmlContent = htmlContent.replace(
        /<div[^>]*class=["'][^"']*(?:header-bar|toolbar|header-container)["'][^>]*>[\s\S]*?<\/div>/gi,
        ""
      );
    ...
    `;
      ...
      const message =
        error instanceof Error ? error.message : "Unknown Python preview error";
      // Ensure CORS headers are set even on error
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate Python preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post("/api/preview/js", uploadDocument.single("file"), async (req, res) => {
  ...
  try {
    ...
    const args = [scriptPath, jsPath, outputPath, "--max-size-mb", "10"];
    ...
    // Wrap HTML in styled template with JavaScript viewer toolbar
    const styledHtml = `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Unknown JavaScript preview error";
    // Ensure CORS headers are set even on error
    res.set({
      "Access-Control-Allow-Origin": "*",
      "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    ...
    res
      .status(500)
      .json({ error: `Failed to generate JavaScript preview: ${message}` });
  } finally {
    await fs
      .rm(tmpDir, { recursive: true, force: true })
      .catch(() => undefined);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const args = [scriptPath, cssPath, outputPath, "--max-size-mb", "10"];
    ...
    `;
      ...
      const message =
        error instanceof Error ? error.message : "Unknown CSS preview error";
      // Ensure CORS headers are set even on error
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate CSS preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const args = [scriptPath, htmlPath, outputPath, "--max-size-mb", "10"];
    ...
    `;
      ...
      const message =
        error instanceof Error ? error.message : "Unknown HTML preview error";
      // Ensure CORS headers are set even on error
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate HTML preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
// XML Preview endpoint - OPTIONS for CORS preflight
app.options("/api/preview/xml", (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!scriptExists) {
        console.error(`Python script not found: ${scriptPath}`);
        res.status(500).json({ error: "XML preview script not found" });
        return;
      }

      const args = [scriptPath, xmlPath, htmlPath, "--max-size-mb", "10"];
      ...
      <body>
        ...
        <script>
          ...
          function updateZoom() {
            xmlContent.style.transform = \`scale(\${currentZoom / 100})\`;
            xmlContent.style.transformOrigin = 'top left';
            document.getElementById('zoom-level').textContent = \`\${currentZoom}%\`;
          }
          ...
          function fitWidth() {
            const containerWidth = window.innerWidth - 80;
            const contentWidth = xmlContent.scrollWidth || 1200;
            currentZoom = Math.floor((containerWidth / contentWidth) * 100);
            currentZoom = Math.max(50, Math.min(300, currentZoom)); // Clamp between 50% and 300%
            updateZoom();
          }
          ...
          function previousPage() {
            // XML is single view, so this is disabled
          }
    ...
    `;
      ...
      const message =
        error instanceof Error ? error.message : "Unknown XML preview error";
      // Ensure CORS headers are set even on error
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate XML preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const args = [scriptPath, jsonPath, htmlPath, "--max-size-mb", "10"];
    ...
    `;
      ...
      const message =
        error instanceof Error ? error.message : "Unknown JSON preview error";
      // Ensure CORS headers are set even on error
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate JSON preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const args = [scriptPath, odsPath, htmlPath, "--max-rows", "2000"];
      ...
      if (
        ...
        stdout.includes("CONVERSION FAILED")
      ) {
        ...
        if (stderr.includes("ValueError:") || stdout.includes("ValueError:")) {
          const valueErrorMatch = (stderr + stdout).match(
            /ValueError:\s*(.+?)(?:\n|$)/i
          );
          if (valueErrorMatch) {
          ...
          stderr.includes("BadZipFile") ||
          stderr.includes("not a zip file")
        ) {
          errorMessage =
            "The uploaded file is not a valid ODS file. ODS files must be ZIP archives containing OpenDocument Spreadsheet data.";
          ...
          if (errorLines.length > 0) {
            ...
            if (lastError.startsWith("ERROR:") || lastError.includes("Error")) {
              errorMessage =
                lastError.replace(/^ERROR:\s*/i, "").trim() || errorMessage;
            }
          }
        }
    ...
    `;
      ...
      const message =
        error instanceof Error ? error.message : "Unknown ODS preview error";
      // Ensure CORS headers are set even on error
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate ODS preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
// CSV Preview endpoint - OPTIONS for CORS preflight
app.options("/api/preview/csv", (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!scriptExists) {
        console.error(`Python script not found: ${scriptPath}`);
        res.status(500).json({ error: "CSV preview script not found" });
        return;
      }

      const args = [scriptPath, csvPath, htmlPath, "--max-rows", "2000"];
      ...
      <body>
        ...
        <script>
          ...
          function updateZoom() {
            csvContent.style.transform = \`scale(\${currentZoom / 100})\`;
            csvContent.style.transformOrigin = 'top left';
            document.getElementById('zoom-level').textContent = \`\${currentZoom}%\`;
          }
          ...
          function fitWidth() {
            const containerWidth = window.innerWidth - 80;
            const contentWidth = csvContent.scrollWidth || 1200;
            currentZoom = Math.floor((containerWidth / contentWidth) * 100);
            currentZoom = Math.max(50, Math.min(300, currentZoom)); // Clamp between 50% and 300%
            updateZoom();
          }
          ...
          function previousPage() {
            // CSV is single view, so this is disabled
          }
    ...
    `;
      ...
      const message =
        error instanceof Error ? error.message : "Unknown CSV preview error";
      // Ensure CORS headers are set even on error
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate CSV preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
// Excel Preview endpoint - OPTIONS for CORS preflight
app.options("/api/preview/xlsx", (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!scriptExists) {
        console.error(`Python script not found: ${scriptPath}`);
        res.status(500).json({ error: "Excel preview script not found" });
        return;
      }

      const args = [scriptPath, xlsxPath, htmlPath, "--max-rows", "2000"];
      ...
      <body>
        ...
        <script>
          ...
          function updateZoom() {
            excelContent.style.transform = \`scale(\${currentZoom / 100})\`;
            excelContent.style.transformOrigin = 'top left';
            document.getElementById('zoom-level').textContent = \`\${currentZoom}%\`;
          }
          ...
          function fitWidth() {
            const containerWidth = window.innerWidth - 80;
            const contentWidth = excelContent.scrollWidth || 1200;
            currentZoom = Math.floor((containerWidth / contentWidth) * 100);
            currentZoom = Math.max(50, Math.min(300, currentZoom)); // Clamp between 50% and 300%
            updateZoom();
          }
          ...
          function previousPage() {
            // Excel is single view, so this is disabled
          }
    ...
    `;
      ...
      const message =
        error instanceof Error ? error.message : "Unknown Excel preview error";
      // Ensure CORS headers are set even on error
      res.set({
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      ...
      res
        .status(500)
        .json({ error: `Failed to generate Excel preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "ODP execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown ODP preview error";
      res
        .status(500)
        .json({ error: `Failed to generate ODP preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "OTP execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown OTP preview error";
      res
        .status(500)
        .json({ error: `Failed to generate OTP preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "POT execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown POT preview error";
      res
        .status(500)
        .json({ error: `Failed to generate POT preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const fileExt =
        file.originalname.split(".").pop()?.toLowerCase() || "pps";
      ...
      if (!scriptExists) {
        ...
        return res
          .status(500)
          .json({ error: "PPS/PPSX preview script not found" });
      }

      const args = [scriptPath, ppsPath, outputPath];

      console.log("Executing PPS/PPSX script:", {
        pythonPath,
        scriptPath,
        args,
      });
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "PPS/PPSX execution failed";
      }

      if (stdout.trim().length > 0)
        console.log("PPS/PPSX stdout:", stdout.trim());
      if (stderr.trim().length > 0)
        console.warn("PPS/PPSX stderr:", stderr.trim());
      ...
      if (!outputExists) {
        throw new Error(
          `PPS/PPSX script did not produce preview: ${outputPath}`
        );
      }
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Unknown PPS/PPSX preview error";
      res
        .status(500)
        .json({ error: `Failed to generate PPS/PPSX preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Determine file extension
      const fileExt =
        file.originalname.split(".").pop()?.toLowerCase() || "ppt";
      const inputFileName = `input.${fileExt}`;
      ...
      if (!scriptExists) {
        ...
        return res
          .status(500)
          .json({ error: "PPT/PPTX preview script not found" });
      }

      const args = [scriptPath, pptPath, outputPath];

      console.log("Executing PPT/PPTX script:", {
        pythonPath,
        scriptPath,
        args,
      });
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "PPT/PPTX execution failed";
      }
      ...
      if (timedOut) {
        throw new Error(
          "PowerPoint conversion timed out. The file may be too large or complex."
        );
      }
      ...
      if (
        ...
        throw new Error(
          `PPT/PPTX conversion failed: ${stderr.substring(0, 500)}`
        );
      }
      ...
      if (!outputExists) {
        ...
        console.error(
          "Output file not found. Directory contents:",
          dirContents
        );
        throw new Error(
          `PPT/PPTX script did not produce preview. Check logs for details.`
        );
      }
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Unknown PPT/PPTX preview error";
      res
        .status(500)
        .json({ error: `Failed to generate PowerPoint preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "SDD execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown SDD preview error";
      res
        .status(500)
        .json({ error: `Failed to generate SDD preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "STI execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown STI preview error";
      res
        .status(500)
        .json({ error: `Failed to generate STI preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
app.post("/api/preview/sx", uploadDocument.single("file"), async (req, res) => {
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Unknown SX preview error";
    res
      .status(500)
      .json({ error: `Failed to generate SX preview: ${message}` });
  } finally {
    await fs
      .rm(tmpDir, { recursive: true, force: true })
      .catch(() => undefined);
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        stdout = execError.stdout || "";
        stderr =
          execError.stderr || execError.message || "UOP execution failed";
      }
      ...
      const message =
        error instanceof Error ? error.message : "Unknown UOP preview error";
      res
        .status(500)
        .json({ error: `Failed to generate UOP preview: ${message}` });
    } finally {
      await fs
        .rm(tmpDir, { recursive: true, force: true })
        .catch(() => undefined);
...
const convertCsvToTomlPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python TOML script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->TOML error`;
...
const convertCsvToXmlPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python XML script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->XML error`;
...
const convertCsvToYamlPython = async (
  ...
  try {
    ...
    const args = [
      ...
      options.indent || "2",
      "--default-flow-style",
      options.defaultFlowStyle || "false",
      "--allow-unicode",
      options.allowUnicode || "true",
    ];
    ...
    if (!outputExists) {
      throw new Error(
        `Python YAML script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->YAML error`;
...
const convertCsvToEpubPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python EPUB script did not produce output file: ${outputPath}`
      );
    }
...
const convertCsvToSqlPython = async (
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python SQL script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->SQL error`;
...
const convertCsvToParquetPython = async (
  ...
  const tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "morphy-csv-parquet-")
  );
  const originalBase = path.basename(
    file.originalname,
    path.extname(file.originalname)
  ...
  try {
    ...
    if (!outputExists) {
      throw new Error(
        `Python Parquet script did not produce output file: ${outputPath}`
      );
    }
    ...
    const message =
      error instanceof Error ? error.message : `Unknown CSV->Parquet error`;
...
async function convertDocToEpubPython(
  ...
  try {
    ...
    const args = [scriptPath, docPath, outputPath];
    ...
    const message =
      error instanceof Error ? error.message : `Unknown DOC->EPUB error`;
      ...
      if (result.status === "fulfilled") {
        ...
        if (value.storedFilename && value.downloadUrl) {
          ...
          return {
            ...
            outputFilename:
              value.filename ||
              files[index].originalname.replace(/\.doc$/i, ".epub"),
            size: value.size || 0,
            success: false,
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.avro$/i, ".json")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "AVRO to JSON conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        results.push({
          ...
          size: outputBuffer.length,
          success: true,
          downloadPath: `data:application/json;base64,${outputBuffer.toString(
            "base64"
          )}`,
        });
      } catch (error) {
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.avro$/i, ".ndjson")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            const outputBuffer = await fs.readFile(outputPath);
            console.log(
              "AVRO to NDJSON: Output file size:",
              outputBuffer.length
            );
            res.set({
            ...
            console.error(
              "AVRO to NDJSON conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        results.push({
          ...
          size: outputBuffer.length,
          success: true,
          downloadPath: `data:application/x-ndjson;base64,${outputBuffer.toString(
            "base64"
          )}`,
        });
      } catch (error) {
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.csv$/i, ".avro")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "CSV to AVRO conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        results.push({
          ...
          size: outputBuffer.length,
          success: true,
          downloadPath: `data:application/avro;base64,${outputBuffer.toString(
            "base64"
          )}`,
        });
      } catch (error) {
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.bmp$/i, ".webp")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "BMP to WebP conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        try {
          ...
          console.error(
            "BMP to WebP batch: Script does not exist:",
            scriptPath
          );
          results.push({
            originalName: file.originalname,
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "BMP to WebP batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "BMP to WebP batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                ...
                console.error(
                  "BMP to WebP batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "BMP to WebP batch conversion error for file:",
          file.originalname,
          error
        );
...
app.post(
  ...
  async (req, res) => {
    ...
    const timeoutHandler = () => {
      ...
      if (!res.headersSent) {
        res.status(408).json({
          ...
          message:
            "CR2 to ICO conversion is taking longer than expected. Please try with a smaller file or contact support.",
          timeout: true,
        });
      }
    };
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.cr2$/i, ".ico")
      );
      ...
      console.log(
        "CR2 to ICO: Icon size received:",
        iconSize,
        "Type:",
        typeof iconSize
      );
      console.log(
        'CR2 to ICO: Icon size check (iconSize !== "default"):',
        iconSize !== "default"
      );
      ...
      if (iconSize && iconSize !== "default") {
        ...
        console.log(
          "CR2 to ICO: Using ORIGINAL image size (no --sizes parameter)"
        );
      }
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "CR2 to ICO conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        console.log(
          "CR2 to ICO batch: iconSize value:",
          req.body.iconSize,
          "type:",
          typeof req.body.iconSize
        );
        ...
        if (req.body.iconSize && req.body.iconSize !== "default") {
          ...
          console.log(
            "CR2 to ICO batch: Using custom size:",
            req.body.iconSize
          );
        } else {
          console.log(
            "CR2 to ICO batch: Using ORIGINAL image size (no --sizes parameter)"
          );
        }

        console.log(
          "CR2 to ICO batch: Final Python command:",
          pythonArgs.join(" ")
        );
        const python = spawn("/opt/venv/bin/python", pythonArgs);
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "CR2 to ICO batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "CR2 to ICO batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                  ...
                  size: outputBuffer.length,
                  success: true,
                  downloadPath: `data:image/x-icon;base64,${outputBuffer.toString(
                    "base64"
                  )}`,
                });
                resolve();
                ...
                console.error(
                  "CR2 to ICO batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "CR2 to ICO batch conversion error for file:",
          file.originalname,
          error
        );
        // Error already pushed to results in the promise rejection
    ...
    const timeoutHandler = () => {
      ...
      if (!res.headersSent) {
        res.status(408).json({
          error: "Conversion timeout",
          message:
            "DNG to WebP conversion is taking longer than expected. Please try with a smaller file or contact support.",
          timeout: true,
        });
      }
    };
...
app.post(
  ...
  async (req, res) => {
    ...
    const timeoutHandler = () => {
      ...
      if (!res.headersSent) {
        res.status(408).json({
          error: "Conversion timeout",
          message:
            "CR2 to WebP conversion is taking longer than expected. Please try with a smaller file or contact support.",
          timeout: true,
        });
      }
    };
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.cr2$/i, ".webp")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "CR2 to WebP conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        try {
          ...
          console.error(
            "CR2 to WebP batch: Script does not exist:",
            scriptPath
          );
          results.push({
            originalName: file.originalname,
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "CR2 to WebP batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "CR2 to WebP batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                ...
                console.error(
                  "CR2 to WebP batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "CR2 to WebP batch conversion error for file:",
          file.originalname,
          error
        );
        // Error already pushed to results in the promise rejection
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.eps$/i, ".webp")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "EPS to WebP conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        try {
          ...
          console.error(
            "EPS to WebP batch: Script does not exist:",
            scriptPath
          );
          results.push({
            originalName: file.originalname,
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "EPS to WebP batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "EPS to WebP batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                ...
                console.error(
                  "EPS to WebP batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "EPS to WebP batch conversion error for file:",
          file.originalname,
          error
        );
        results.push({
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.epub$/i, ".csv")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "EPUB to CSV conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        try {
          ...
          console.error(
            "EPUB to CSV batch: Script does not exist:",
            scriptPath
          );
          results.push({
            originalName: file.originalname,
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "EPUB to CSV batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "EPUB to CSV batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                  ...
                  size: outputBuffer.length,
                  success: true,
                  downloadPath: `data:text/csv;base64,${outputBuffer.toString(
                    "base64"
                  )}`,
                });
                resolve();
                ...
                console.error(
                  "EPUB to CSV batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "EPUB to CSV batch conversion error for file:",
          file.originalname,
          error
        );
        results.push({
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.docx$/i, ".csv")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "DOCX to CSV conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        try {
          ...
          console.error(
            "DOCX to CSV batch: Script does not exist:",
            scriptPath
          );
          results.push({
            originalName: file.originalname,
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "DOCX to CSV batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "DOCX to CSV batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                ...
                console.error(
                  "DOCX to CSV batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "DOCX to CSV batch conversion error for file:",
          file.originalname,
          error
        );
        results.push({
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.docx$/i, ".epub")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "DOCX to EPUB conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        try {
          ...
          console.error(
            "DOCX to EPUB batch: Script does not exist:",
            scriptPath
          );
          results.push({
            originalName: file.originalname,
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "DOCX to EPUB batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "DOCX to EPUB batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                  ...
                  size: outputBuffer.length,
                  success: true,
                  downloadPath: `data:application/epub+zip;base64,${outputBuffer.toString(
                    "base64"
                  )}`,
                });
                resolve();
                ...
                console.error(
                  "DOCX to EPUB batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "DOCX to EPUB batch conversion error for file:",
          file.originalname,
          error
        );
        results.push({
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.docx$/i, ".mobi")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "DOCX to MOBI conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        try {
          ...
          console.error(
            "DOCX to MOBI batch: Script does not exist:",
            scriptPath
          );
          results.push({
            originalName: file.originalname,
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "DOCX to MOBI batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "DOCX to MOBI batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                  ...
                  size: outputBuffer.length,
                  success: true,
                  downloadPath: `data:application/x-mobipocket-ebook;base64,${outputBuffer.toString(
                    "base64"
                  )}`,
                });
                resolve();
                ...
                console.error(
                  "DOCX to MOBI batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "DOCX to MOBI batch conversion error for file:",
          file.originalname,
          error
        );
        results.push({
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.docx$/i, ".odt")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "DOCX to ODT conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        try {
          ...
          console.error(
            "DOCX to ODT batch: Script does not exist:",
            scriptPath
          );
          results.push({
            originalName: file.originalname,
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "DOCX to ODT batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "DOCX to ODT batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                  ...
                  size: outputBuffer.length,
                  success: true,
                  downloadPath: `data:application/vnd.oasis.opendocument.text;base64,${outputBuffer.toString(
                    "base64"
                  )}`,
                });
                resolve();
                ...
                console.error(
                  "DOCX to ODT batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "DOCX to ODT batch conversion error for file:",
          file.originalname,
          error
        );
        results.push({
...
// Route: DOC to ODT (Single)
app.post(
  "/convert/doc-to-odt/single",
  upload.single("file"),
  async (req, res) => {
    // Set CORS headers
    res.set({
    ...
    console.log("DOC->ODT single conversion request");

    const tmpDir = path.join(os.tmpdir(), `doc-odt-${Date.now()}`);
    ...
    try {
      ...
      const scriptPath = path.join(__dirname, "..", "scripts", "doc_to_odt.py");
      console.log("DOC to ODT: Executing Python script:", scriptPath);
      console.log("DOC to ODT: Input file:", inputPath);
      console.log("DOC to ODT: Output file:", outputPath);

      // Check if script exists
      try {
        await fs.access(scriptPath);
        console.log("DOC to ODT: Script exists");
      } catch (error) {
        console.error("DOC to ODT: Script does not exist:", scriptPath);
        return res.status(500).json({ error: "Conversion script not found" });
      }
      ...
      python.stdout.on("data", (data: Buffer) => {
        ...
        console.log("DOC to ODT stdout:", data.toString("utf-8"));
      });

      python.stderr.on("data", (data: Buffer) => {
        stderr += data.toString("utf-8");
        console.log("DOC to ODT stderr:", data.toString("utf-8"));
      });

      python.on("close", async (code: number) => {
        console.log("DOC to ODT: Python script finished with code:", code);
        console.log("DOC to ODT: stdout:", stdout);
        console.log("DOC to ODT: stderr:", stderr);
        ...
        try {
          if (
            ...
            const outputBuffer = await fs.readFile(outputPath);
            console.log("DOC to ODT: Output file size:", outputBuffer.length);
            res.set({
              "Content-Type": "application/vnd.oasis.opendocument.text",
            ...
            console.error(
              "DOC to ODT conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
...
// Route: DOC to ODT (Batch)
app.post("/convert/doc-to-odt/batch", uploadBatch, async (req, res) => {
  // Set CORS headers
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
  ...
  console.log("DOC->ODT batch conversion request");

  const tmpDir = path.join(os.tmpdir(), `doc-odt-batch-${Date.now()}`);
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        console.log("DOC to ODT batch: Executing Python script:", scriptPath);
        console.log("DOC to ODT batch: Input file:", inputPath);
        console.log("DOC to ODT batch: Output file:", outputPath);

        try {
          await fs.access(scriptPath);
          console.log("DOC to ODT batch: Script exists");
        } catch (error) {
          console.error("DOC to ODT batch: Script does not exist:", scriptPath);
          results.push({
            originalName: file.originalname,
            outputFilename: "",
            size: 0,
        ...
        python.stdout.on("data", (data: Buffer) => {
          stdout += data.toString("utf-8");
          console.log("DOC to ODT batch stdout:", data.toString("utf-8"));
        });

        python.stderr.on("data", (data: Buffer) => {
          stderr += data.toString("utf-8");
          console.log("DOC to ODT batch stderr:", data.toString("utf-8"));
        });

        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "DOC to ODT batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "DOC to ODT batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                ...
                console.error(
                  "DOC to ODT batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
    ...
    console.error("DOC to ODT batch error:", error);
    res.set({
      "Access-Control-Allow-Origin": "*",
      "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.docx$/i, ".txt")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "DOCX to TXT conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        try {
          ...
          console.error(
            "DOCX to TXT batch: Script does not exist:",
            scriptPath
          );
          results.push({
            originalName: file.originalname,
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "DOCX to TXT batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "DOCX to TXT batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                  ...
                  size: outputBuffer.length,
                  success: true,
                  downloadPath: `data:text/plain;base64,${outputBuffer.toString(
                    "base64"
                  )}`,
                });
                resolve();
                ...
                console.error(
                  "DOCX to TXT batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "DOCX to TXT batch conversion error for file:",
          file.originalname,
          error
        );
        results.push({
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.doc$/i, ".csv")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "DOC to CSV conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "DOC to CSV batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "DOC to CSV batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                ...
                console.error(
                  "DOC to CSV batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "DOC to CSV batch conversion error for file:",
          file.originalname,
          error
        );
        results.push({
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.doc$/i, ".epub")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "DOC to EPUB conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        try {
          ...
          console.error(
            "DOC to EPUB batch: Script does not exist:",
            scriptPath
          );
          results.push({
            originalName: file.originalname,
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "DOC to EPUB batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "DOC to EPUB batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                ...
                console.error(
                  "DOC to EPUB batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "DOC to EPUB batch conversion error for file:",
          file.originalname,
          error
        );
        results.push({
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.doc$/i, ".mobi")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "DOC to MOBI conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        try {
          ...
          console.error(
            "DOC to MOBI batch: Script does not exist:",
            scriptPath
          );
          results.push({
            originalName: file.originalname,
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "DOC to MOBI batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "DOC to MOBI batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                ...
                console.error(
                  "DOC to MOBI batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "DOC to MOBI batch conversion error for file:",
          file.originalname,
          error
        );
        results.push({
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.doc$/i, ".txt")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "DOC to TXT conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "DOC to TXT batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "DOC to TXT batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                ...
                console.error(
                  "DOC to TXT batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "DOC to TXT batch conversion error for file:",
          file.originalname,
          error
        );
        results.push({
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.(heic|heif)$/i, ".svg")
      );
      ...
      python.on("close", async (code: number) => {
        ...
        try {
          if (
            ...
            console.error(
              "HEIC to SVG conversion failed. Code:",
              code,
              "Stderr:",
              stderr
            );
    ...
    for (const file of files) {
      try {
        ...
        try {
          ...
          console.error(
            "HEIC to SVG batch: Script does not exist:",
            scriptPath
          );
          results.push({
            originalName: file.originalname,
        ...
        await new Promise<void>((resolve, reject) => {
          python.on("close", async (code: number) => {
            console.log(
              "HEIC to SVG batch: Python script finished with code:",
              code
            );
            ...
            try {
              if (
                ...
                const outputBuffer = await fs.readFile(outputPath);
                console.log(
                  "HEIC to SVG batch: Output file size:",
                  outputBuffer.length
                );
                results.push({
                  originalName: file.originalname,
                  ...
                  size: outputBuffer.length,
                  success: true,
                  downloadPath: `data:image/svg+xml;base64,${outputBuffer.toString(
                    "base64"
                  )}`,
                });
                resolve();
                ...
                console.error(
                  "HEIC to SVG batch conversion failed. Code:",
                  code,
                  "Stderr:",
                  stderr
                );
        ...
        console.error(
          "HEIC to SVG batch conversion error for file:",
          file.originalname,
          error
        );
        results.push({
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "HEIC to PDF: Failed to create temp directory:",
          mkdirError
        );
        res.set({
          "Access-Control-Allow-Origin": "*",
        ...
        try {
          if (!res.headersSent) {
            if (
              ...
              console.error(
                "HEIC to PDF conversion failed. Code:",
                code,
                "Stderr:",
                stderr
              );
...
// Route: HEIF to PNG (Single)
app.post(
  "/convert/heif-to-png/single",
  upload.single("file"),
  async (req, res) => {
    res.set({
      "Access-Control-Allow-Origin": "*",
    ...
    console.log("HEIF->PNG single conversion request");
    const tmpDir = path.join(os.tmpdir(), `heif-png-${Date.now()}`);
    try {
      const file = req.file;
      if (!file) {
        return res.status(400).json({ error: "No file uploaded" });
      ...
      let stdout = "",
        stderr = "";
      python.on("error", async (err: Error) => {
        console.error("HEIF->PNG single: failed to start python:", err);
        if (!res.headersSent) {
          res.set({
            "Access-Control-Allow-Origin": "*",
            "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
...
// Route: HEIF to PNG (Batch) - OPTIONS for CORS preflight
app.options("/convert/heif-to-png/batch", (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
...
// Route: HEIF to PNG (Batch)
app.post("/convert/heif-to-png/batch", uploadBatch, async (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
  ...
  console.log("HEIF->PNG batch conversion request");
  const tmpDir = path.join(os.tmpdir(), `heif-png-batch-${Date.now()}`);
  try {
    const files = req.files as Express.Multer.File[];
    if (!files || files.length === 0) {
      return res.status(400).json({ error: "No files uploaded" });
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.(heic|heif)$/i, ".webp")
      );
...
// Route: HEIC to TIFF (Single) - OPTIONS for CORS preflight
app.options("/convert/heic-to-tiff/single", (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
...
// Route: HEIC to TIFF (Single)
app.post(
  "/convert/heic-to-tiff/single",
  upload.single("file"),
  async (req, res) => {
    res.set({
      "Access-Control-Allow-Origin": "*",
    ...
    console.log("HEIC->TIFF single conversion request");

    const tmpDir = path.join(os.tmpdir(), `heic-tiff-${Date.now()}`);
    ...
    try {
      ...
      const outputPath = path.join(
        tmpDir,
        file.originalname.replace(/\.(heic|heif)$/i, ".tiff")
      );
      ...
      const quality = parseInt(req.body.quality) || 95;
      const maxDimension = parseInt(req.body.maxDimension) || 4096;
      const compression = req.body.compression || "tiff_lzw";
      ...
      python.on("error", async (error: Error) => {
        console.error("HEIC to TIFF: Failed to start Python process:", error);
        if (!res.headersSent) {
          res.set({
            "Access-Control-Allow-Origin": "*",
            "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
      ...
      python.stdout.on("data", (data: Buffer) => {
        stdout += data.toString();
        console.log("HEIC to TIFF stdout:", data.toString());
      });

      python.stderr.on("data", (data: Buffer) => {
        stderr += data.toString();
        console.log("HEIC to TIFF stderr:", data.toString());
      });

      const timeout = setTimeout(async () => {
        console.error("HEIC to TIFF: Conversion timeout after 5 minutes");
        python.kill();
        if (!res.headersSent) {
          res.set({
            "Access-Control-Allow-Origin": "*",
      ...
      python.on("close", async (code: number) => {
        clearTimeout(timeout);
        console.log("HEIC to TIFF: Python script finished with code:", code);
        console.log("HEIC to TIFF: stdout:", stdout);
        console.log("HEIC to TIFF: stderr:", stderr);
        ...
        try {
          if (!res.headersSent) {
            if (
              ...
              console.error(
                "HEIC to TIFF conversion failed. Code:",
                code,
                "Stderr:",
                stderr
              );
          ...
          console.error("Error handling conversion result (TIFF):", error);
          if (!res.headersSent) {
            res.set({
              "Access-Control-Allow-Origin": "*",
              "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
...
// Route: HEIC to TIFF (Batch) - OPTIONS for CORS preflight
app.options("/convert/heic-to-tiff/batch", (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
...
// Route: HEIC to TIFF (Batch)
app.post("/convert/heic-to-tiff/batch", uploadBatch, async (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
  ...
  console.log("HEIC->TIFF batch conversion request");
  const tmpDir = path.join(os.tmpdir(), `heic-tiff-batch-${Date.now()}`);
            ...
            try {
              if (
                ...
                results.push({
                  ...
                  size: outputBuffer.length,
                  success: true,
                  downloadPath: `data:image/tiff;base64,${outputBuffer.toString(
                    "base64"
                  )}`,
                });
              } else {
...
app.post("/api/compress/jpg", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const optimize =
      req.body.optimize !== "false" && req.query.optimize !== "false";
    ...
    const outputPath = path.join(
      tmpDir,
      file.originalname.replace(/\.(jpg|jpeg)$/i, "_compressed.jpg")
    );
    ...
    python.on("close", async (code: number) => {
      ...
      try {
        if (
          ...
          console.log(
            "JPG Compression: Compressed size:",
            compressedSize,
            "bytes"
          );
...
app.post("/api/compress/png", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const outputPath = path.join(
      tmpDir,
      file.originalname.replace(/\.(png)$/i, "_compressed.png")
    );
    ...
    python.on("close", async (code: number) => {
      ...
      try {
        if (
          ...
          console.log(
            "PNG Compression: Compressed size:",
            compressedSize,
            "bytes"
          );
...
app.post("/api/compress/pdf", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const outputPath = path.join(
      tmpDir,
      file.originalname.replace(/\.(pdf)$/i, "_compressed.pdf")
    );
    ...
    console.log("PDF Compression: Executing Python script:", scriptPath);
    console.log("PDF Compression: Input file:", inputPath);
    console.log("PDF Compression: Output file:", outputPath);
    console.log("PDF Compression: Quality:", quality);
    console.log("PDF Compression: Optimize:", optimize);

    // Check if script exists
    try {
      await fs.access(scriptPath);
      console.log("PDF Compression: Script exists");
    } catch (error) {
      console.error("PDF Compression: Script does not exist:", scriptPath);
      return res.status(500).json({ error: "Compression script not found" });
    }
    ...
    python.stdout.on("data", (data: Buffer) => {
      stdout += data.toString();
      console.log("PDF Compression stdout:", data.toString());
    });

    python.stderr.on("data", (data: Buffer) => {
      stderr += data.toString();
      console.log("PDF Compression stderr:", data.toString());
    });

    python.on("close", async (code: number) => {
      console.log("PDF Compression: Python script finished with code:", code);
      console.log("PDF Compression: stdout:", stdout);
      console.log("PDF Compression: stderr:", stderr);
      ...
      try {
        if (
          ...
          console.log("PDF Compression: Original size:", originalSize, "bytes");
          console.log(
            "PDF Compression: Compressed size:",
            compressedSize,
            "bytes"
          );
          console.log("PDF Compression: Savings:", savingsPercent, "%");
...
// Route: PDF Compression (Batch)
app.post("/api/compress/pdf/batch", uploadBatch, async (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
  ...
  console.log("PDF compression batch request");

  const tmpDir = path.join(os.tmpdir(), `pdf-compress-batch-${Date.now()}`);
  ...
  try {
    ...
    console.error("PDF Compression batch error:", error);
    const message = error instanceof Error ? error.message : "Unknown error";
    res.status(500).json({ error: message });
    await fs
      .rm(tmpDir, { recursive: true, force: true })
...
// CORS preflight for PDF compression endpoints
app.options("/api/compress/pdf", (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
...
app.options("/api/compress/pdf/batch", (req, res) => {
  res.set({
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, Accept",
...
app.post("/api/youtube/transcript", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Transcript extraction script not found" });
    }
    ...
    python.on("close", async (code: number) => {
      console.log(
        "YouTube Transcript: Python script finished with code:",
        code
      );
      ...
      try {
        ...
        if (stdout && stdout.trim()) {
          try {
            ...
            if (result.success) {
              ...
              console.log(
                "YouTube Transcript: Returning 400 error:",
                result.error
              );
              res.status(400).json({
                success: false,
            ...
            console.error(
              "YouTube Transcript: Failed to parse JSON output:",
              parseError
            );
        ...
        if (code === 0) {
          ...
          const errorMessage =
            stderr || stdout || "Transcript extraction failed";
          res.status(500).json({
            success: false,
            error: errorMessage,
            code: code,
